...
class PlotText(Annotation):
    ...
    def _resolve_align(self) -> Literal["left", "center", "right"]:
    ...
    def _resolve_baseline(self) -> Literal["top", "middle", "bottom"]:
        head = self.position.split("-")[0]
        if head == "top":
            return "top"
        if head == "bottom":
            return "bottom"
    ...
    def build_layer(
        ...
        *,
        ...
        Called by ``render_annotations`` which threads through the
        rendered chart's width and height.
        """
        if not self.text:
            return alt.Chart(pd.DataFrame({"_": []})).mark_point()

        align = self._resolve_align()
...
def _wrap_text_to_width(text: str, width_px: int, font_size: int) -> str:
    ...
    English at typical body sizes (10-12px). Long single words wider
    than the budget are emitted on their own line (no mid-word
    breaking). Explicit ``\\n`` characters in the input are honoured
    as paragraph breaks, and each paragraph is wrapped independently.
    """
    if not text:
        return ""
...
def _build_text_panel(
    ...
    *,
    ...
    panel = (
        ...
        .properties(width=width, height=h)
    )
    # Strip the default border / background that Vega-Lite might draw
    # around an empty-data chart so the panel reads as inline text, not
    # a framed sub-chart.
    return panel
...
def _resolve_side_width(
    ...
    interpreted as raw pixels. ``None`` falls back to ``default_pct``.
    """
    raw = cfg.get("width_pct")
    if raw is None:
        return int(chart_width * default_pct)
    val = float(raw)
    if val <= 1.0:
    ...
    *,
    ...
    if cap_cfg is not None:
        cap_panel = _build_text_panel(
            cap_cfg, width=chart_width, height=None, skin_config=skin_config,
        )
        body = alt.vconcat(body, cap_panel, spacing=4).resolve_scale(
    ...
    return alt.hconcat(*h_parts, spacing=12).resolve_scale(
        color="independent", x="independent", y="independent",
    ).resolve_legend(color="independent")
...
def _apply_text_panels_to_spec(
    ...
    *,
    ...
    def _panel_spec(cfg: Dict[str, Any], width: int, height: Optional[int]) -> Dict[str, Any]:
        panel = _build_text_panel(
            cfg, width=width, height=height, skin_config=skin_config,
        )
        d = panel.to_dict()
        d.pop("$schema", None)
        d.pop("config", None)
    ...
    if needs_caption:
        ...
        cap_spec = _panel_spec(cfg_to_use, chart_width, cap_h)
        body = {"vconcat": [body, cap_spec], "spacing": 4}
    ...
    if needs_left or needs_right:
        ...
        if needs_left:
            ...
            h_parts.append(_panel_spec(cfg_to_use, left_w, chart_height))
        h_parts.append(body)
        if needs_right:
            ...
            h_parts.append(_panel_spec(cfg_to_use, right_w, chart_height))
        wrapped: Dict[str, Any] = {"hconcat": h_parts, "spacing": 12}
    else:
        wrapped = body
