...
Do **not** draft **Trader Hat Tip** sections: no invented trader first-person voice, views, positioning, or trade ideas. Those are supplied by the desk/traders (or left for humans to paste). Do not write **trader notes** or attributed quotes that purport to come from named traders. You may apply structure (headings, spacing) only when the human has provided the substantive text.
...
Human- or trader-authored only. 1-2 trader hat tips per issue. Each features a named trader from a specific desk providing their market view, positioning, and trade ideas. The format is conversational and first-person -- this is the trader talking directly to clients.
...
- At least one Trader Hat Tip (substance from traders/desk, not model-generated)
...
- **Thoughts:** dominant theme; Monday–Friday facts (data, auctions, Fed, levels, curve closes); forward week calendar  
- **Not So Standard Deviation:** top rates and cross-asset movers (product, z-score, change) and GIR source date  
- **Trader Hat Tip(s):** full text from desk/traders only (the model does not write this)  
- **Econ snippet / Data recap / Deep dive:** topic, excerpts, numbers, exhibit pointers  

Rough bullets or pasted research are enough; the rest can be drafted from what you supply.
