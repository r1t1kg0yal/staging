...
A dataset entry can declare a `compute` block listing named expressions evaluated against an existing source dataset. The compiler runs an AST-level whitelist (no `eval`, no `__import__`, no attribute access, no string/list/dict literals) over a small set of arithmetic + windowed helpers, materialises each output column, and auto-stamps `field_provenance` with `system: "computed"`, the recipe string, the upstream column list, and a units inference (e.g. `* 100` on percent-units inputs -> `bp`; `zscore(...)` -> `z`; `pct_change(...)` -> `percent`; otherwise inherited when every upstream column shares units).
...
After compile, `manifest.datasets.spreads.source` carries the original date / inherited columns plus every computed output, and `manifest.datasets.spreads.field_provenance` has one entry per output column with the recipe, upstream refs, and inferred units. The entire `compute` block can also be attached to a dataset that already has a `source` -- the computed columns are appended to the existing source rather than emitted as a new dataset.
...
```python
"compute": {
    "real_yield_10y": "rates.us_10y - inflation.breakeven_10y"
}
```

**Column-name constraint.** The AST parser treats bare column names as Python identifiers, so column references in expressions must start with a letter or underscore (not a digit) and must not contain spaces / dots / dashes. Pre-rename problematic columns upstream (`df.rename(columns={"10y_real": "real_10y"})`) or compute the value inline from a reference that does parse (`zscore(us_10y - 2.2, 60)` rather than `zscore(\`10y_real\`, 60)`).

This eliminates the most common kind of `build.py` boilerplate (compute spread / ratio / z-score / index columns, then ship them as a dataset). The recipe survives into the popup Sources footer (`system: computed`, `recipe: <expr>`), so the reader can trace any chart point back to its formula.

**Worked example in the gallery.** `rates_monitor` ships a `rates_derived` entry that computes `us_10y_z_60 = zscore(us_10y, 60)` over the rates panel; the resulting chart on the Macro overlay tab carries an auto-populated provenance block (`system: "computed"`, `recipe: "zscore(us_10y, 60)"`, `computed_from: ["us_10y"]`, `units: "z"`). The popup Sources footer surfaces the recipe directly so the reader can trace any chart point back to its formula.
...
### 2.3 Chart types (29 builders + `raw` passthrough)

> The 29 entries below are the registered builders -- valid values for `spec.chart_type` inside dashboard chart widgets. The 30th conceptual type, `raw`, is **not** a `chart_type` value; it's the dashboard's `option` / `ref` widget variants (3.4) and the `make_echart(option=..., chart_type='raw')` passthrough call. Use those when you need an ECharts option the builder catalogue doesn't cover.
...
| `tree`             | `name`, `parent`                                          |
| `waterfall`        | `x` (category), `y` (signed delta), optional `is_total` (bool column flagging totals from-zero). P&L bridges, factor decomp. |
| `slope`            | `x` (snapshot col with exactly 2 distinct values), `y`, `color`. Side-by-side comparison of N categories at start vs end. |
| `fan_cone`         | `x`, `y` (central path), `bands` = list of `{lower, upper, label?, opacity?, color?}`. Forecast cones, scenario ribbons. |
| `marimekko`        | `x` (column-axis cat), `y` (row-axis cat), `value` (cell magnitude), optional `order_x`, `order_y`. 2D categorical proportions. |
...
Two complementary paths: server-side (headless Chrome, runs from Python with no browser open) and browser-side (live "Download Panel" button in the header of every compiled dashboard). They serve different workflows and are independent.
...
| 5        | `Download Panel`     | always (one file: the entire current view as a single PNG)         |
...
| `pivot`     | `id`, `dataset_ref`, `row_dim_columns`, `col_dim_columns`, `value_columns` | Crosstab with row / col / value / agg dropdowns over a long-form dataset. See 3.6a. |
...
Common optional fields: `w` (1-12 grid span), `h_px` (chart only; default 280), `title`, `show_when` (compile / runtime conditional visibility -- see 3.5.1), `initial_state` (per-widget opening defaults that bypass the always-start-in-raw drawer state -- see 3.5.2).

**Widget presentation knobs (apply to every tile type):**

| Field            | Purpose                                                                                   |
|------------------|-------------------------------------------------------------------------------------------|
| `title`          | Card header text. Always lives at the widget level (sibling of `spec`), never inside `spec` -- the validator rejects `spec.title` / `spec.subtitle`. Every tile type (chart, KPI, table, markdown) shares the same uniform header contract. PNG export bakes the widget title back into the chart option automatically. |
...
#### 3.5.1 `show_when` -- conditional widget visibility

Adaptive dashboards: a widget can declare a `show_when` condition; the compiler removes the widget entirely when a *data* condition fails (no diagnostics, no target counts, no filter wiring -- the widget is gone), and the JS runtime toggles CSS display when a *filter* condition fails (widget is in the DOM but invisible until the predicate matches).

Three clause shapes, freely composable via `all` / `any`:

```json
"show_when": {"data": "rates.latest.vix > 25"}
"show_when": {"filter": "scope", "value": "domestic"}
"show_when": {"filter": "lookback", "in": ["6M", "1Y"]}
"show_when": {"filter": "z_threshold", "op": ">=", "value": 2.0}
"show_when": {"all": [
    {"data": "market.latest.vix < 30"},
    {"filter": "scope", "in": ["us", "eu"]}
]}
```

Data clauses follow the dotted KPI source shape (`dataset.aggregator.column`), with `==` / `!=` / `>` / `>=` / `<` / `<=` / `contains` / `startsWith` / `endsWith` ops. Filter clauses reference declared filter ids; the validator emits a path-specific error when a typo points at a non-existent filter. The compiler short-circuits on data clauses; runtime evaluation only kicks in for filter-bearing conditions.

Use case examples: `risk` notes that only render when vol is elevated; `Federal-Reserve` panels that only render under `scope == "domestic"`; methodology blurbs that only appear when the user is in the `expert` mode toggle.

#### 3.5.2 Per-widget `initial_state` -- opening drawer defaults

Every chart / table / KPI tile carries a `⋮` controls drawer that mutates state at runtime. `initial_state` seeds those state objects so the tile opens with the desired view -- a chart in YoY %, a table sorted by Z desc, a KPI on its 1m compare period. Mirrors the drawer state shape; unknown keys are ignored, so adding a knob to the drawer doesn't require updating every author's manifest.
...
"spec": {
  ...
  "initial_state": {
    "transform": "yoy_pct",       // yoy_pct / pct_change / change / log /
                                    // zscore / rolling_zscore_252 / rank_pct
                                    // / index100 / yoy_change / annualized_change / ytd
    "smoothing": 5,
    "y_scale": "log",
    "y_range": "from_zero",
...
#### 3.5.3 In-cell visualisations (table column `in_cell`)

Adds a horizontal data bar or inline sparkline to a numeric column without sacrificing the formatted text. Bars are normalised against the **visible-row** range (after filter + search), so the encoding tracks what's actually on screen. Sparklines read from a sibling dataset filtered to the row's lookup key.
...
Bars: dual-direction when the column crosses zero (anchored at 50%), single-direction otherwise. Override colors via `bar_color_pos` / `bar_color_neg`. Sparklines: 80x16 SVG with a colored end-cap (green when up vs first point, red when down). Set `show_text: false` to hide the cell's number entirely; otherwise the bar/spark sits next to the formatted value.

#### 3.5.4 Auto-computed stat strip (Σ button -> popup)

Every supported time-series chart (`line`, `multi_line`, `area`) gets a **Σ button** in its toolbar (left of the `⋮` controls drawer toggle). Clicking it opens a popup showing one row per visible series with current value, deltas at standard horizons (`1d` / `5d` / `1m` / `3m` / `YTD` / `1Y`), 1Y high-low range, and 1Y percentile rank. The popup is computed on-demand each click, so it always reflects the current chart state -- including any drawer transforms or filter selections.

We deliberately do not render the strip inline above each chart: stacking that band on every time-series tile became visually cluttering. The popup-on-click pattern keeps the chrome quiet by default and surfaces the stats only when the reader actively asks.
...
| `z` / `zscore`          | arithmetic      | `1.8  Δ5d +0.6`               |
| `pp`                    | pp arithmetic   | `+18.4%  Δ5d +1.2pp`          |
| (missing)               | magnitude heuristic | falls back to pct          |

The popup renders one row per series with no upper limit (modal is scrollable). Per-spec overrides:
...
The Σ button is auto-suppressed for chart types where the strip doesn't apply (anything other than `line` / `multi_line` / `area`), so authors don't need to opt out per chart type. The popup degrades gracefully when units are missing -- the magnitude heuristic picks `bp` for [0, 25] tight bands, `arith` for series that cross zero with small magnitude, `pct_abs` for 4-digit values, otherwise `pct`. Always set `field_provenance.units` for clean output.
...
### 3.6a Pivot widget (`widget: pivot`)

Crosstab over a long-form dataset where the viewer picks the row dim, column dim, value column, and aggregator from author-supplied whitelists. Use this when the analyst's natural question is "show me sector x country breakdown of YTD perf, actually pivot it by sector x decile of beta, actually agg on median" -- one widget instead of three pre-baked tables.
...
| `color_scale` | no | `"sequential"` (single-direction blue ramp), `"diverging"` (red-white-blue around 0), `"auto"` (diverging when data crosses 0, else sequential), or a `{min, max, palette}` dict. |
| `show_totals` | no | Append row/column totals (recomputed from the underlying values). Default `true`. |

Filters that target the pivot's `dataset_ref` flow through naturally -- the table recomputes against the filtered subset on every filter change. The user's last dropdown selections survive URL state encoding (`#p.<id>.r=...&p.<id>.c=...`) so a shared link reproduces the exact pivot view.
...
#### 3.7.3 Cascading filters (`depends_on` + `options_from`)

A filter can declare `depends_on: <upstream_filter_id>` plus `options_from: {dataset, key, where?}`. When the upstream filter changes, the dependent filter rebuilds its options from the named dataset, optionally filtered by a `where` clause that substitutes upstream filter values via `${filter_id}` syntax.
...
Supported `where` ops: `==`, `!=`, `>`, `>=`, `<`, `<=`. The dependent filter's existing value is preserved when it remains valid in the new option set; otherwise it falls back to the first new option (or empty for `multiSelect`). Cascades chain through arbitrary depth; when an upstream filter changes, every direct dependent rebuilds, and broadcast() propagates the change downstream.

Validation: `depends_on` must reference a declared filter id; self-dependency raises an error. `options_from` requires both `dataset` and `key`. Empty initial `options` lists are accepted -- the runtime populates them on first paint.
...
- **Download PNG / CSV / XLSX** — direct downloads of the chart image and underlying data. PNG bakes the title in (same path as the dashboard-level "Download Charts" button); XLSX uses the SheetJS bundle that's already loaded for the dashboard-level Excel export.
...
Optional `manifest.header_actions[]` appends custom buttons/links to the header (left of the Methodology / Refresh / Download Charts / Download Panel / Download Excel buttons). Use for dashboard-specific escape hatches -- docs, related dashboards, run-a-script hooks, etc. Buttons all share a single visual treatment (white in light mode, light-blue in dark mode) -- the manifest cannot override the styling.
...
- **Per-tile PNG** -- each chart tile has a `PNG` button in its toolbar (also includes the title / subtitle in the export, same as Download Charts).
...
- **In-cell bars / sparklines** -- per-column `in_cell: "bar"` adds a horizontal data bar inside the cell (anchored to column-range, dual-direction when values cross zero); `in_cell: "sparkline"` reads from a sibling dataset and draws an inline 80x16 SVG line per row, with a colored end-cap. Tooltips/sort/conditional formatting still apply. See 3.5.3.
- **Auto stat strip** -- every line / multi_line / area chart gets a Σ button in its toolbar; clicking opens a popup with current value, deltas at 1d / 5d / 1m / 3m / YTD / 1Y horizons, 1Y range, 1Y percentile rank, one row per visible series. Computed on-demand at each click so the popup always reflects the current drawer / filter state. Format choice (bp / pct + abs / pp / arithmetic) follows `field_provenance.units` per column; falls back to a magnitude heuristic when units are missing. Override via `spec.stat_strip = {horizons, delta_format, show_range, show_percentile}` or suppress with `spec.stat_strip: false`. See 3.5.4.
- **URL state (shareable views)** -- active tab + filter values + per-chart drawer state (transform, smoothing, scale) + table sort/search + KPI compare period + pivot dropdowns serialise to the URL hash on every change and restore on load. Send a colleague the URL and they get the same view. Coexists with the existing `localStorage` tab-persistence fallback for users who clear hash navigation.
- **Conditional widget visibility** -- widgets with a `show_when` data clause are removed at compile time when the underlying data fails the predicate; widgets with a `show_when` filter clause are emitted but their CSS display is toggled at runtime as filter state changes. See 3.5.1.
- **Cascading filters** -- a filter that declares `depends_on: <upstream_filter_id>` + `options_from: {dataset, key, where?}` rebuilds its options automatically when the upstream changes. Cascades chain through any depth. See 3.7.3.
- **Pivot dropdowns** -- every `widget: pivot` ships its row / column / value / aggregator dropdowns wired to a JS-side reducer over the dataset. Filters that target the pivot's `dataset_ref` flow through naturally; user dropdown selections survive URL state encoding. See 3.6a.
- **Per-widget initial state** -- chart / table / KPI tiles can declare `initial_state` to seed the controls drawer (e.g. open a chart in YoY %, open a table sorted by Z desc, open a KPI on its 1m compare period). See 3.5.3.
...
- Widget-specific required fields enforced:
    ...
    - `pivot` requires `dataset_ref` + non-empty `row_dim_columns` / `col_dim_columns` / `value_columns`
...
- Filter `depends_on`, when set, must reference a declared filter id and cannot self-reference; `options_from` requires `dataset` + `key`.
- Widget `show_when`, when set, must be a dict with one of `data` / `filter` / `all` / `any` keys; filter-clause `filter` ids must reference declared filter ids.
- Table column `in_cell`, when set, must be `bar` / `sparkline` / `heat`; `sparkline` additionally requires `from_dataset` + `row_key`.
...
| Report / email PNG snapshot of a compiled dashboard                 | `compile_dashboard(..., save_pngs=True)` for per-widget PNGs, or the browser-side `Download Panel` button |
...
`demos.py` bundles 15 end-to-end **dashboard** scenarios that collectively exercise every PRISM-facing capability in the stack -- 30 chart types, 9 widget kinds (incl. `pivot`), 9 filter types, 4 brush types, 4 sync values, 5 annotation types, 3 chart-widget variants (`spec` / `ref` / `option`), persistence + diagnostics workflow, the prose / markdown surfaces, and every new feature added to the manifest schema (computed datasets, show_when, cascading filters, in-cell viz, initial_state, auto stat strip, URL state).

**Chart-driven**

| Name                  | Highlights                                                                                                        |
|-----------------------|-------------------------------------------------------------------------------------------------------------------|
| `rates_monitor`       | 8-tab rates monitor (US curve + 5 global central banks), brush, dual-axis, gauges, summary banner + 4 note kinds. Also exercises **field_provenance + auto stat strip** on the 2s10s spread chart, a **manifest-level `compute` block** producing a 60d rolling z-score column (`rates_derived`), a **`show_when: data` risk note** that only renders when 2s10s is inverted, and **`initial_state` opening the 10Y chart in YoY change mode**. |
| `risk_regime`         | `correlation_matrix` (dedicated builder), `scatter_multi`, VIX term by regime, factor boxplots, drawdown band     |
| `fomc_monitor`        | Cut-prob gauge, FFR candlestick, dot-plot strokeDash, voter radar, decision calendar, `arrow` annotation. Adds a **`fan_cone` rate-path forecast** (central path + 25-75% / 10-90% confidence ribbons) to the Dots & path tab. |
...
| `portfolio_analytics` | Multi-asset book: VaR gauge, factor radar, allocation pie, parallel_coords, histogram; `pinned` KPI ribbon, `kv` section. Adds an **Attribution & allocation** section with a **`waterfall` MTD P&L bridge**, a **`marimekko` bucket x size mosaic**, and a **`pivot` widget** over a long-form returns panel (book / region / window). |
| `markets_wrap`        | 17-chart cross-asset wrap, click-emit-filter, conditional-formatted top movers; `image` widget, `divider`s, `legend` sync, filter `"*"` wildcard. The top-movers table now uses **`in_cell: bar`** on the 1D / MTD / YTD columns and **`in_cell: sparkline`** for a 60d price history per row (reads from a sister `mov_hist` dataset). Adds a **`slope` chart** for sector MTD vs YTD between the sector bar and the rates section. |
| `screener_studio`     | Three-tab toolbox: rich RV screen, all 9 filter types, bond drilldown w/ rich modal; `dateRange mode="filter"`. **`initial_state`** opens the RV table sorted by `z` desc with 2-decimal numeric formatting; the bond drilldown filter bar adds a **cascading `Issuer` select** (`depends_on: bs_sector`, `options_from: bonds.issuer where sector == ${bs_sector}`). |
...
| `feature_studio`  | **Canonical reference dashboard** -- single-page tour with EVERY new manifest feature in isolation, one tab per feature group: (1) auto stat strip + per-widget `initial_state` + computed-dataset spread chart, (2) all four new chart types side-by-side (waterfall / slope / fan_cone / marimekko), (3) `show_when` risk note + `in_cell: bar` + `in_cell: sparkline` table + `pivot` widget, (4) `depends_on` + `options_from` cascading filter pair (region -> country). The other demos exercise these features in their natural production context; this demo isolates each one for quick reference. |

**Feature -> demo coverage matrix.** The gallery's top panel auto-generates a coverage table from each registry entry's `features` list. To run only the demos exercising one feature, use the standard `--demo <name>` filter on `demos.py`. Coverage as of the current registry:

| Feature              | Demos that exercise it                                                                 |
|----------------------|----------------------------------------------------------------------------------------|
| `stat_strip`         | rates_monitor, risk_regime, fomc_monitor, equity_deep_dive, portfolio_analytics, markets_wrap, macro_studio, feature_studio |
| `url_state`          | (free for every dashboard -- no opt-in)                                               |
| `computed_datasets`  | rates_monitor, feature_studio                                                          |
| `show_when`          | rates_monitor, feature_studio                                                          |
| `cascading_filters`  | screener_studio, feature_studio                                                        |
| `in_cell_bar`        | markets_wrap, feature_studio                                                           |
| `in_cell_sparkline`  | markets_wrap, feature_studio                                                           |
| `waterfall`          | portfolio_analytics, feature_studio                                                    |
| `slope`              | markets_wrap, feature_studio                                                           |
| `fan_cone`           | fomc_monitor, feature_studio                                                           |
| `marimekko`          | portfolio_analytics, feature_studio                                                    |
| `initial_state`      | rates_monitor, screener_studio, feature_studio                                         |
| `pivot` widget       | portfolio_analytics, feature_studio                                                    |
...
**How `gallery.html` is composed.** `_gallery_html()` emits a self-contained, GS-branded card-grid page with three sections, top to bottom:

1. **Header bar** -- engine / theme / palette pills + pass count.
2. **New-feature coverage matrix** -- one row per new feature listing every demo that exercises it. Auto-generated from each registry entry's `features` list (`url_state` is treated as covered by every successful demo, since it's free for every compiled dashboard). Use this panel to find the simplest demo that demonstrates the feature you're after.
3. **Card grid** -- one card per demo, in `DEMO_REGISTRY` order.
...
| feature chips | small pill row above the description listing every new feature this demo exercises (e.g. `waterfall` / `marimekko` / `pivot` for `portfolio_analytics`). Demos with no new features show a muted `no new features` chip so the absence is explicit. |
...
- The viewport is fixed at the supplied `(width, height)`; full scroll-height auto-detect is not supported on the server-side path. For a true full-page capture use the browser-side `Download Panel` button (Section 2.9.2) instead.
