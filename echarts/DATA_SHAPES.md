...
## 12.1 Computed datasets (replace build.py glue with manifest expressions)

Spreads, ratios, z-scores, and index-100 normalisations don't have to be computed in Python and shipped as separate columns. A dataset entry can declare a `compute` block of named expressions that the compiler evaluates at compile time, materialises into the dataset's source, and auto-stamps with `field_provenance.system: "computed"`. PRISM gets:

- shorter `build.py` (no pandas glue for derived columns)
- automatic provenance: every computed cell carries its recipe in the popup Sources footer
- inferred units: `* 100` on `percent`-units inputs -> `bp`; `zscore(...)` -> `z`; `pct_change(...)` -> `percent`; otherwise inherited when every upstream column shares units

```python
manifest = {
    "datasets": {
        "rates": df_rates,                       # has us_2y, us_10y, etc.
        "spreads": {
            "from": "rates",                     # source dataset
            "compute": {
...
Allowed function names: `log`, `log10`, `log2`, `exp`, `sqrt`, `abs`, `sign`, `round`, `mean`, `std`, `min`, `max`, `sum`, `zscore(x, window?)`, `rolling_mean(x, n)`, `rolling_std(x, n)`, `pct_change(x, periods?)`, `diff(x, periods?)`, `shift(x, periods?)`, `clip(x, lo?, hi?)`, `index100(x)`, `rank_pct(x)`. Anything else (especially attribute access like `df.values`, list/dict literals, `__import__`) raises a validation error -- the AST whitelist is strict by design.

Cross-dataset refs use `<other_ds>.<col>` notation: `"real_yield_10y": "rates.us_10y - inflation.breakeven_10y"`.

**When to use compute blocks vs build.py:**
- Use compute when the formula is one expression and the inputs all live in one dataset. Cleaner manifest, automatic provenance.
- Use `build.py` when the transformation requires multi-step pandas (groupby + agg + merge), conditional logic, or a Python helper. Compute blocks are deliberately minimal -- they're not a general DSL.
