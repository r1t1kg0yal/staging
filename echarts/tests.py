...
class TestThemes(unittest.TestCase):
    def test_gs_themes(self):
        # Canonical light theme + its dark twin. The dark twin is
        # required so the always-on theme toggle in the dashboard
        # shell has a target to flip to without manifest involvement.
        names = sorted(THEMES.keys())
        self.assertEqual(names, ["gs_clean", "gs_clean_dark"])
    ...
    def test_dark_theme_inverts_surface(self):
        light = get_theme("gs_clean")["echarts"]
        dark = get_theme("gs_clean_dark")["echarts"]
        # Backgrounds should not match -- the whole point of the dark
        # twin is that the chart canvas blends with the dark chrome.
        self.assertNotEqual(light["backgroundColor"],
                              dark["backgroundColor"])
        self.assertNotEqual(light["textStyle"]["color"],
                              dark["textStyle"]["color"])

    def test_dark_theme_palette_is_brightened(self):
        # In dark mode every dark brand colour (GS_NAVY, GS_FOREST,
        # GS_BURGUNDY, GS_BRICK, GS_STEEL, GS_SLATE, GS_PLUM, GS_GOLD)
        # is swapped for a brightened twin so chart series stay
        # visible against the dark navy surface. Slot order matches
        # gs_primary so series identity carries across the toggle.
        from config import (
            GS_NAVY_BRIGHT, GS_SKY, GS_GOLD_BRIGHT, GS_BURGUNDY_BRIGHT,
            GS_FOREST_BRIGHT, GS_SLATE_BRIGHT, GS_PLUM_BRIGHT,
            GS_AMBER_BRIGHT, GS_BRICK_BRIGHT, GS_STEEL_BRIGHT,
        )
        dark_palette = get_theme("gs_clean_dark")["echarts"]["color"]
        self.assertEqual(
            dark_palette,
            [GS_NAVY_BRIGHT, GS_SKY, GS_GOLD_BRIGHT, GS_BURGUNDY_BRIGHT,
             GS_FOREST_BRIGHT, GS_SLATE_BRIGHT, GS_PLUM_BRIGHT,
    ...
    def test_dark_brand_colors_are_brighter(self):
        # Sanity check that each *_BRIGHT token is genuinely brighter
        # than its light counterpart. Catches regressions where a
        # bright variant accidentally lands the same as or darker
        # than the dark surface.
        from config import (
            GS_DARK_SURFACE,
            GS_NAVY, GS_NAVY_BRIGHT, GS_FOREST, GS_FOREST_BRIGHT,
            GS_BURGUNDY, GS_BURGUNDY_BRIGHT, GS_BRICK, GS_BRICK_BRIGHT,
            GS_STEEL, GS_STEEL_BRIGHT,
        )
        ...
        bg = _luma(GS_DARK_SURFACE)
        pairs = [
            (GS_NAVY, GS_NAVY_BRIGHT),
            (GS_FOREST, GS_FOREST_BRIGHT),
            (GS_BURGUNDY, GS_BURGUNDY_BRIGHT),
            (GS_BRICK, GS_BRICK_BRIGHT),
            (GS_STEEL, GS_STEEL_BRIGHT),
        ]
        for light, bright in pairs:
            self.assertGreater(_luma(bright), _luma(light),
                                 msg=f"{bright} not brighter than {light}")
            # Each bright variant should be visibly above the dark
            # surface luminance (>= 2x is a loose-but-meaningful gate
            # for line/bar visibility).
            self.assertGreater(_luma(bright), bg * 2,
                                 msg=f"{bright} too close to bg {GS_DARK_SURFACE}")
...
class TestSpecToOption(unittest.TestCase):
    ...
    def test_title_subtitle_not_consumed_from_spec(self):
        """Titles and subtitles live at the widget level only; the
        spec-to-option lowering ignores any title / subtitle keys
        passed in the spec (the validator rejects them upstream, but
        the lowering is also defensive). The resulting option's
        title block is therefore empty."""
        opt = _spec_to_option(
            {"chart_type": "line", "dataset": "rates",
              "mapping": {"x": "date", "y": "us_10y"},
        ...
        self.assertEqual(opt["title"]["text"], "")
        self.assertEqual(opt["title"]["subtext"], "")
...
class TestChartDataDiagnostics(unittest.TestCase):
    ...
    def test_compile_recovers_from_chart_build_failure(self):
        """Spec that would fail to build gets an error-severity diagnostic
        + a placeholder; sibling charts still compile. The error may be
        caught either pre-build (chart_mapping_*) or at build time
        (chart_build_failed) -- both paths preserve the resilience
        contract: bad chart fails loudly via the diagnostic stream,
        good charts in the same dashboard still render.
        """
        m = self._wrap(
        ...
        self.assertTrue(r.success)
        self.assertIsNotNone(r.html)
        broken_diags = [d for d in r.diagnostics
                         if d.widget_id == "broken"
                         and d.severity == "error"]
        self.assertGreater(
            len(broken_diags), 0,
            "broken widget did not produce any error-severity "
            "diagnostic; resilience contract violated"
        )
        # The good chart still rendered
        self.assertIn("chart-good", r.html)

    def test_compile_skips_build_when_pre_build_already_flagged(self):
        """When chart_data_diagnostics already emits an error-severity
        diagnostic for a widget, _resolve_chart_specs should skip the
        actual builder call -- the build would crash for the same
        reason, producing a redundant chart_build_failed next to the
        already-actionable pre-build diagnostic. The placeholder still
        renders, using the pre-build diagnostic's triage payload.
        """
        m = self._wrap(
            datasets={"d": pd.DataFrame(
                {"x": [["us"], ["eu"], ["jp"]],
                 "y": ["A", "B", "A"], "v": [1.0, 2.0, 3.0]})},
            layout={"rows": [[
                {"widget": "chart", "id": "broken", "w": 12,
        ...
        self.assertIn("chart_mapping_column_unhashable_cells", codes)
        # No redundant build-failed diagnostic -- pre-build path won.
        self.assertNotIn("chart_build_failed", codes)
    ...
    def test_diagnostic_str_promotes_fix_hint_inline(self):
        """``Diagnostic.__str__`` is the rendering surfaced via
        ``DashboardResult.warnings`` and grep-driven workflows; the
        most actionable piece of context (``fix_hint``) needs to be
        on that line so PRISM's iteration prompts can pattern-match
        without parsing JSON.
        """
        d = Diagnostic(severity="error",
                         code="chart_mapping_column_unhashable_cells",
                         widget_id="hm",
                         path="layout.rows[0][0].spec.mapping.x",
                         message="x has list-valued cells",
                         context={"fix_hint": "df.explode('x')",
                                    "available_columns": ["x", "y"]})
        s = str(d)
        self.assertIn("fix:", s)
        self.assertIn("df.explode('x')", s)
        # full context still recoverable via to_dict / format_verbose
        self.assertIn("available_columns", d.format_verbose())

    def test_unhashable_list_cells_pre_build_diagnostic(self):
        """The exact bug behind '(no data) unhashable type: list' in
        the dashboards: a column referenced by mapping has list-valued
        cells. The pre-build hygiene check must catch it with an
        actionable diagnostic before the heatmap builder hits the
        ``{v: i for ...}`` dict comprehension at echart_studio.py:2816.
        """
        df = pd.DataFrame({
            "x": [["us", "ig"], ["us", "hy"], ["eu", "ig"], ["eu", "hy"]],
            "y": ["A", "B", "A", "B"],
            "v": [1.0, 2.0, 3.0, 4.0],
        })
        m = self._wrap(
            datasets={"d": df},
            layout={"rows": [[{"widget": "chart", "id": "hm", "w": 12,
                                "spec": {"chart_type": "heatmap",
                                          "dataset": "d",
                                          "mapping": {"x": "x", "y": "y",
                                                      "value": "v"}}}]]})
        diags = chart_data_diagnostics(m)
        unh = [d for d in diags
               if d.code == "chart_mapping_column_unhashable_cells"]
        self.assertEqual(len(unh), 1)
        d = unh[0]
        self.assertEqual(d.severity, "error")
        self.assertEqual(d.widget_id, "hm")
        self.assertEqual(d.context["mapping_key"], "x")
        self.assertEqual(d.context["column"], "x")
        self.assertEqual(d.context["cell_type"], "list")
        self.assertEqual(d.context["bad_count"], 4)
        self.assertIn("fix_hint", d.context)
        self.assertIn("explode", d.context["fix_hint"])
        # placeholder + diagnostic share the same human_message
        self.assertIn("list-valued", d.context["human_message"])

    def test_unhashable_dict_cells_pre_build_diagnostic(self):
        """Same hygiene check should also fire on dict cells (and
        set, frozenset, bytearray) -- those are the other unhashable
        scalar containers Python's hash protocol rejects.
        """
        df = pd.DataFrame({
            "category": [{"k": "us"}, {"k": "eu"}, {"k": "us"}],
            "value": [1.0, 2.0, 3.0],
        })
        m = self._wrap(
            datasets={"d": df},
            layout={"rows": [[{"widget": "chart", "id": "p", "w": 12,
                                "spec": {"chart_type": "pie",
                                          "dataset": "d",
                                          "mapping": {
                                              "category": "category",
        ...
        unh = [d for d in diags
               if d.code == "chart_mapping_column_unhashable_cells"]
        self.assertEqual(len(unh), 1)
        self.assertEqual(unh[0].context["cell_type"], "dict")
        self.assertIn("json_normalize", unh[0].context["fix_hint"])

    def test_unhashable_check_does_not_false_positive(self):
        """Ordinary str / int / datetime / NaN columns must NOT fire
        the unhashable-cells diagnostic. Otherwise every heatmap on
        the dashboard suddenly grows a spurious error.
        """
        df = pd.DataFrame({
            "x": ["a", "b", "a", "b", None],
            "y": [1, 2, 3, 4, 5],
        ...
        self.assertNotIn("chart_mapping_column_unhashable_cells", codes)

    def test_builder_exception_translator_known_patterns(self):
        """The shared translation table maps every cryptic substring
        we've seen in production to a (human_message, fix_hint) pair.
        Both surfaces (placeholder subtext + Diagnostic.context) read
        from this table, so adding a new pattern automatically
        upgrades both at once.
        """
        from echart_dashboard import _translate_builder_exception
        # Unhashable-list crash from heatmap dict comp
        t = _translate_builder_exception(TypeError("unhashable type: 'list'"))
        self.assertEqual(t["exception_type"], "TypeError")
        self.assertIn("list-valued", t["human_message"])
        self.assertIn("explode", t["fix_hint"])
        # Pandas length-mismatch fallout from `df[df[col] == g]`
        t = _translate_builder_exception(
            ValueError(("Lengths must match to compare", (6,), (1,)))
        )
        self.assertIn("color/group", t["human_message"])
        # Numeric-coercion fallout
        t = _translate_builder_exception(
            ValueError("could not convert string to float: 'two'")
        )
        self.assertIn("non-numeric", t["human_message"])
        # Unknown exception still returns the structured baseline
        t = _translate_builder_exception(RuntimeError("kaboom"))
        self.assertEqual(t["exception_type"], "RuntimeError")
        self.assertEqual(t["exception_message"], "kaboom")
        self.assertNotIn("human_message", t)
        self.assertNotIn("fix_hint", t)

    def test_placeholder_uses_human_message_from_translator(self):
        """When the chart card's subtext used to read 'unhashable
        type: list', the user couldn't tell which chart, which column,
        or how to fix it. The placeholder now leans on the
        translator's ``human_message`` so the rendered card carries
        the same plain-English triage as the diagnostic stream.
        """
        from echart_dashboard import _empty_placeholder_option
        opt = _empty_placeholder_option(
            TypeError("unhashable type: 'list'"),
            chart_type="heatmap",
        )
        sub = opt["title"]["subtext"]
        self.assertIn("heatmap", sub)
        self.assertIn("list-valued", sub)
        self.assertNotIn("unhashable type", sub)
...
def build_ghost_data() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "rates_line",
                "title": 'Rates curve (empty source)', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "multi_line", "dataset": "rates",
                            "mapping": {"x": "date",
                                        "y": ["us_2y", "us_10y"]},}},
                {"widget": "chart", "id": "rates_bar",
                "title": 'Rates bar (empty source)', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "bar", "dataset": "rates",
                            "mapping": {"x": "date", "y": "us_2y"},}},
            ],
            [
                {"widget": "chart", "id": "flow_sk",
                "title": 'Flow sankey (empty source)', "w": 6, "h_px": 380,
                  "spec": {"chart_type": "sankey", "dataset": "flows",
                            "mapping": {"source": "src",
                                        "target": "tgt",
                                        "value": "v"},}},
                {"widget": "chart", "id": "share_pie",
                "title": 'Share pie (empty source)', "w": 6, "h_px": 380,
...
def build_column_typos() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "rates_curve",
                "title": 'UST curve (column typos)', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "multi_line", "dataset": "rates",
                            "mapping": {"x": "date",
                                        "y": ["us_2y", "us_10y"]},}},
                {"widget": "chart", "id": "two_year",
                "title": 'US 2Y (column typo)', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "line", "dataset": "rates",
                            "mapping": {"x": "date", "y": "us_2y"},}},
            ],
            [
...
def build_nan_blizzard() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "allnan_line",
                "title": 'All-NaN y column', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "line", "dataset": "all_nan",
                            "mapping": {"x": "date", "y": "us_2y"},}},
                {"widget": "chart", "id": "mostly_line",
                "title": 'Mostly-NaN y column', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "line", "dataset": "mostly_nan",
                            "mapping": {"x": "date", "y": "us_2y"},}},
            ],
            [
                {"widget": "chart", "id": "allnan_bar",
                "title": 'All-NaN bar', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "bar", "dataset": "all_nan",
                            "mapping": {"x": "date", "y": "us_10y"},}},
                {"widget": "chart", "id": "mostly_multi",
                "title": 'Mostly-NaN multi-line', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "multi_line",
                            "dataset": "mostly_nan",
                            "mapping": {"x": "date",
                                        "y": ["us_2y", "us_10y"]},}},
...
def build_type_mismatch() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "junk_line",
                "title": "Numeric column has 'n/a' / 'ERR'", "w": 6, "h_px": 320,
                  "spec": {"chart_type": "line", "dataset": "string_y",
                            "mapping": {"x": "date", "y": "us_2y"},}},
                {"widget": "chart", "id": "junk_bar",
                "title": 'Bar w/ string y values', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "bar", "dataset": "string_y",
                            "mapping": {"x": "date", "y": "us_2y"},}},
            ],
            [
                {"widget": "chart", "id": "bad_dates_line",
                "title": "X axis is 'today' / 'tomorrow'", "w": 6,
                  "h_px": 320,
                  "spec": {"chart_type": "line", "dataset": "bad_dates",
                            "mapping": {"x": "date", "y": "value"},}},
                {"widget": "chart", "id": "bad_dates_scatter",
                "title": 'Size column is non-numeric', "w": 6,
                  "h_px": 320,
...
def build_missing_required() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "pie_no_cat",
                "title": 'Pie missing category', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "pie", "dataset": "pie",
                            "mapping": {"value": "share"},}},
                {"widget": "chart", "id": "donut_no_value",
                "title": 'Donut missing value', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "donut", "dataset": "pie",
                            "mapping": {"category": "region"},}},
            ],
            [
                {"widget": "chart", "id": "sankey_no_tgt",
                "title": 'Sankey missing target', "w": 6, "h_px": 380,
                  "spec": {"chart_type": "sankey", "dataset": "flow",
                            "mapping": {"source": "src", "value": "v"},}},
                {"widget": "chart", "id": "candle_no_close",
                "title": 'Candlestick missing close', "w": 6,
                  "h_px": 380,
            ...
            [
                {"widget": "chart", "id": "treemap_neither",
                "title": 'Treemap: any-of {path,value} or {name,parent,value} not satisfied', "w": 6,
                  "h_px": 320,
                  "spec": {"chart_type": "treemap", "dataset": "pie",
                            "mapping": {"region": "region",
                                        "share": "share"},
}},
                {"widget": "chart", "id": "heatmap_no_value",
                "title": 'Heatmap missing value', "w": 6,
                  "h_px": 320,
...
def build_degenerate_shapes() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "one_row_line",
                "title": 'Line with 1 row', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "line", "dataset": "one_row",
                            "mapping": {"x": "x", "y": "y"},
}},
                {"widget": "chart", "id": "flat_line",
                "title": 'Flat line (constant y)', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "line", "dataset": "flat",
                            "mapping": {"x": "date", "y": "spread_bp"},
}},],
            [
                {"widget": "chart", "id": "neg_pie",
                "title": 'Pie with negative slices', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "pie", "dataset": "neg_pie",
                            "mapping": {"category": "region",
                                        "value": "share"},
}},
                {"widget": "chart", "id": "one_slice",
                "title": 'Donut with one slice = 100%', "w": 6, "h_px": 320,
                  "spec": {"chart_type": "donut", "dataset": "one_slice",
                            "mapping": {"category": "region",
                                        "value": "share"},
}},],
            [
                {"widget": "chart", "id": "inverted_ohlc",
                "title": 'Candlestick: high < low (inverted OHLC)', "w": 12,
                  "h_px": 320,
...
def build_broken_topology() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "self_loop_sk",
                "title": 'Sankey: every flow is a self-loop', "w": 6, "h_px": 380,
                  "spec": {"chart_type": "sankey", "dataset": "self_loops",
                            "mapping": {"source": "src", "target": "tgt",
                                        "value": "v"},
}},
                {"widget": "chart", "id": "disconnect_sk",
                "title": 'Sankey: all values None', "w": 6, "h_px": 380,
                  "spec": {"chart_type": "sankey", "dataset": "disconnect",
                            "mapping": {"source": "src", "target": "tgt",
                                        "value": "v"},
}},],
            [
                {"widget": "chart", "id": "orphan_tree",
                "title": 'Tree: orphan parents', "w": 12, "h_px": 380,
...
def build_broken_bindings() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "rates_curve",
                "title": 'Rates curve (charts are fine)', "w": 12, "h_px": 320,
                  "spec": {"chart_type": "multi_line", "dataset": "rates",
                            "mapping": {"x": "date",
                                        "y": ["us_2y", "us_10y"]},
}},],
    ...
    return _wrap_manifest(
        ...
        layout={"kind": "tabs", "tabs": [
            {"id": "data", "label": "Data shape failures",
              "rows": [
                  [
                      {"widget": "chart", "id": "empty_line",
                      "title": 'Empty dataset', "w": 4,
                        "h_px": 280,
                        "spec": {"chart_type": "line", "dataset": "empty",
                                  "mapping": {"x": "date", "y": "us_2y"},
}},
                      {"widget": "chart", "id": "typo_line",
                      "title": 'Column typos', "w": 4,
                        "h_px": 280,
                        "spec": {"chart_type": "multi_line",
                                  "dataset": "typos",
                                  "mapping": {"x": "date",
                  ...
                  [
                      {"widget": "chart", "id": "junk_y",
                      "title": 'Strings for numbers', "w": 4, "h_px": 280,
                        "spec": {"chart_type": "bar", "dataset": "string_y",
                                  "mapping": {"x": "date", "y": "us_2y"},
}},
                      {"widget": "chart", "id": "neg_pie",
                      "title": 'Negative pie slices', "w": 4, "h_px": 280,
                        "spec": {"chart_type": "pie", "dataset": "neg_pie",
                                  "mapping": {"category": "region",
                                              "value": "share"},
}},
                      {"widget": "chart", "id": "inverted_ohlc",
                      "title": 'Inverted OHLC', "w": 4,
                        "h_px": 280,
                        "spec": {"chart_type": "candlestick",
                                  "dataset": "inverted",
                                  "mapping": {"x": "date", "open": "o",
            ...
            {"id": "topo", "label": "Topology + missing-required",
              "rows": [
                  [
                      {"widget": "chart", "id": "self_loops",
                      "title": 'Sankey self-loops', "w": 6,
                        "h_px": 320,
                        "spec": {"chart_type": "sankey",
                                  "dataset": "self_loop",
                                  "mapping": {"source": "src",
                      ...
                      {"widget": "chart", "id": "pie_no_cat",
                      "title": 'Pie missing category', "w": 6,
                        "h_px": 320,
                        "spec": {"chart_type": "pie", "dataset": "neg_pie",
                                  "mapping": {"value": "share"},
}},],
            ...
            {"id": "bind", "label": "Broken bindings + a control",
              "rows": [
                  [
                      {"widget": "chart", "id": "control",
                      "title": 'Control: this one works', "w": 6, "h_px": 280,
                        "spec": {"chart_type": "multi_line",
                                  "dataset": "real",
                                  "mapping": {"x": "date",
                                              "y": ["us_2y", "us_10y"]},
...
def _build_shape_unclean() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [
            [
                {"widget": "chart", "id": "dti_chart",
                "title": "DTI without 'date' column", "w": 6, "h_px": 280,
                  "spec": {"chart_type": "multi_line",
                            "dataset": "rates_dti",
                            "mapping": {"x": "date",
                                        "y": ["us_2y", "us_10y"]},
}},
                {"widget": "chart", "id": "codes_chart",
                "title": 'Opaque coordinate as column name', "w": 6, "h_px": 280,
                  "spec": {"chart_type": "line",
                            "dataset": "rates_codes",
                            "mapping": {"x": "date",
            ...
            [
                {"widget": "chart", "id": "mi_chart",
                "title": 'MultiIndex columns (compiler does not auto-flatten)', "w": 12, "h_px": 280,
                  "spec": {"chart_type": "line",
                            "dataset": "rates_mi",
...
def _build_size_overflow() -> Dict[str, Any]:
    ...
    return _wrap_manifest(
        ...
        layout={"rows": [[
            {"widget": "chart", "id": "big_line",
            "title": '60K rows of intraday history', "w": 12, "h_px": 320,
...
class TestNewChartTypes(unittest.TestCase):
    """Builders for waterfall / slope / fan_cone / marimekko."""

    def test_waterfall_renders(self):
        import pandas as pd
        df = pd.DataFrame({
            "step": ["Open", "Carry", "Roll", "Curve", "Close"],
            "delta": [100.0, 5.0, -3.0, 8.0, 110.0],
            "is_total": [True, False, False, False, True],
        })
        from echart_studio import make_echart
        r = make_echart(df=df, chart_type="waterfall",
                          mapping={"x": "step", "y": "delta",
                                     "is_total": "is_total"},
                          title="P&L bridge",
                          write_html=False, write_json=False)
        self.assertTrue(r.success, r.error_message)
        self.assertEqual(len(r.option["series"]), 2)  # base + delta

    def test_slope_renders(self):
        import pandas as pd
        df = pd.DataFrame({
            "snapshot": ["start", "end"] * 3,
            "sector": ["Tech", "Tech", "Health", "Health", "Energy", "Energy"],
            "ret": [12.0, 18.4, 4.5, -2.1, -3.0, -8.7],
        })
        from echart_studio import make_echart
        r = make_echart(df=df, chart_type="slope",
                          mapping={"x": "snapshot", "y": "ret",
                                     "color": "sector"},
                          write_html=False, write_json=False)
        self.assertTrue(r.success, r.error_message)
        # 3 sector lines
        self.assertEqual(len(r.option["series"]), 3)

    def test_slope_one_snapshot_raises(self):
        import pandas as pd
        df = pd.DataFrame({
            "snapshot": ["A", "A", "A"],
        ...
        from echart_studio import make_echart
        with self.assertRaises(ValueError):
            make_echart(df=df, chart_type="slope",
                          mapping={"x": "snapshot", "y": "ret",
                                     "color": "sector"},
                          write_html=False, write_json=False)

    def test_fan_cone_renders(self):
        import pandas as pd
        import numpy as np
        n = 30
        df = pd.DataFrame({
            "date": pd.date_range("2026-01-01", periods=n, freq="D"),
            "central": np.linspace(4.0, 4.5, n),
        })
        df["p10"] = df["central"] - 0.30
        df["p25"] = df["central"] - 0.15
        df["p75"] = df["central"] + 0.15
        df["p90"] = df["central"] + 0.30
        from echart_studio import make_echart
        r = make_echart(
            df=df, chart_type="fan_cone",
            mapping={
                "x": "date", "y": "central",
                "bands": [
        ...
        self.assertTrue(r.success, r.error_message)
        # 2 bands × 2 helper series + 1 central path = 5
        self.assertEqual(len(r.option["series"]), 5)

    def test_fan_cone_no_bands_raises(self):
        import pandas as pd
        df = pd.DataFrame({"date": pd.date_range("2026-01-01", periods=3),
                              "central": [1.0, 1.1, 1.2]})
        from echart_studio import make_echart
        with self.assertRaises(ValueError):
            make_echart(df=df, chart_type="fan_cone",
                          mapping={"x": "date", "y": "central", "bands": []},
                          write_html=False, write_json=False)

    def test_marimekko_renders(self):
        import pandas as pd
        df = pd.DataFrame({
            "sector": ["Tech"] * 2 + ["Health"] * 2,
            "size": ["Mega", "Large"] * 2,
            "share": [42.0, 18.0, 14.0, 9.0],
        })
        from echart_studio import make_echart
        r = make_echart(df=df, chart_type="marimekko",
                          mapping={"x": "sector", "y": "size",
                                     "value": "share"},
                          write_html=False, write_json=False)
        self.assertTrue(r.success, r.error_message)


class TestComputedDatasets(unittest.TestCase):
    """Manifest-level compute blocks: synthetic columns from expressions."""
    ...
    def _df(self):
        ...
        return pd.DataFrame({
            "date": pd.date_range("2026-01-01", periods=n, freq="D"),
            "us_2y":  4.10 + np.random.randn(n) * 0.05,
            "us_10y": 4.30 + np.random.randn(n) * 0.05,
        })
    ...
    def test_basic_arithmetic(self):
        ...
        manifest = {
            "schema_version": 1, "id": "ct", "title": "ct",
            "datasets": {
                "rates": self._df(),
                "spread": {"from": "rates", "compute": {
                    "spread_bp": "(us_10y - us_2y) * 100"
                }},
            },
            "layout": {"rows": [[]]},
        }
        _normalize_manifest_datasets(manifest)
        errs = _apply_computed_datasets(manifest)
        self.assertEqual(errs, [])
        spread = manifest["datasets"]["spread"]
        header = spread["source"][0]
        self.assertIn("spread_bp", header)
        # provenance autopopulated
        fp = spread["field_provenance"]
        self.assertEqual(fp["spread_bp"]["system"], "computed")
        self.assertEqual(fp["spread_bp"]["recipe"], "(us_10y - us_2y) * 100")
        self.assertSetEqual(set(fp["spread_bp"]["computed_from"]),
                              {"us_2y", "us_10y"})

    def test_zscore_function(self):
        from echart_dashboard import (
            _normalize_manifest_datasets, _apply_computed_datasets
        )
        manifest = {
            ...
            "datasets": {
                "rates": self._df(),
                "z": {"from": "rates", "compute": {
                    "us_10y_z": "zscore(us_10y)"
                }},
            }, "layout": {"rows": [[]]},
        }
        _normalize_manifest_datasets(manifest)
        errs = _apply_computed_datasets(manifest)
        self.assertEqual(errs, [])
        z_col = manifest["datasets"]["z"]["source"][0].index("us_10y_z")
        zvals = [r[z_col] for r in manifest["datasets"]["z"]["source"][1:]]
        # z-scores should average ~0 and have stdev ~1
        nums = [v for v in zvals if v is not None]
        self.assertGreater(len(nums), 5)
        avg = sum(nums) / len(nums)
        self.assertLess(abs(avg), 0.01)

    def test_dunder_blocked(self):
        """Reject expressions that try to access __import__ or other dunder."""
        from echart_dashboard import (
            _normalize_manifest_datasets, _apply_computed_datasets
        )
        manifest = {
        ...
        _normalize_manifest_datasets(manifest)
        errs = _apply_computed_datasets(manifest)
        self.assertTrue(any("not allowed" in e for e in errs), errs)


class TestShowWhen(unittest.TestCase):
    """Compile-time data conditions + runtime filter conditions."""

    def _manifest(self, vix_values, cond):
        import pandas as pd
        return {
            "schema_version": 1, "id": "sw", "title": "sw",
            "datasets": {"market": pd.DataFrame({
                "date": pd.date_range("2026-01-01", periods=len(vix_values)),
                "vix": vix_values,
            })},
            "layout": {"rows": [
                [{"widget": "note", "id": "n", "w": 12,
                   "kind": "risk", "body": "x", "show_when": cond}],
                [{"widget": "chart", "id": "c", "w": 12,
                   "spec": {"chart_type": "line", "dataset": "market",
                              "mapping": {"x": "date", "y": "vix"}}}],
            ]}
        }

    def test_data_condition_true_keeps_widget(self):
        from echart_dashboard import (
            _apply_show_when_compile, _normalize_manifest_datasets
        )
        m = self._manifest([10, 12, 30], {"data": "market.latest.vix > 25"})
        _normalize_manifest_datasets(m)
        removed = _apply_show_when_compile(m)
        self.assertEqual(removed, 0)

    def test_data_condition_false_drops_widget(self):
        from echart_dashboard import (
            _apply_show_when_compile, _normalize_manifest_datasets
        )
        m = self._manifest([10, 12, 11], {"data": "market.latest.vix > 25"})
        _normalize_manifest_datasets(m)
        removed = _apply_show_when_compile(m)
        self.assertEqual(removed, 1)
        ids = [w["id"] for row in m["layout"]["rows"] for w in row]
        self.assertNotIn("n", ids)

    def test_filter_clause_kept_at_compile(self):
        from echart_dashboard import (
            _apply_show_when_compile, _normalize_manifest_datasets
        )
        m = self._manifest([10, 12], {"filter": "scope", "value": "us"})
        _normalize_manifest_datasets(m)
        removed = _apply_show_when_compile(m)
        self.assertEqual(removed, 0)

    def test_compound_all_short_circuits_on_false(self):
        from echart_dashboard import (
            _apply_show_when_compile, _normalize_manifest_datasets
        )
        cond = {"all": [
            {"data": "market.latest.vix > 25"},   # false (latest=11)
            {"filter": "scope", "value": "us"},
        ]}
        m = self._manifest([10, 11], cond)
        _normalize_manifest_datasets(m)
        removed = _apply_show_when_compile(m)
        self.assertEqual(removed, 1)

    def test_runtime_payload_includes_show_when(self):
        from echart_dashboard import compile_dashboard
        m = self._manifest([10, 11], {"filter": "scope", "value": "us"})
        # Add the filter so validation passes
        m["filters"] = [{"id": "scope", "type": "select", "default": "us",
                          "options": ["us", "eu"], "field": "scope",
                          "targets": ["c"]}]
        r = compile_dashboard(m, write_html=False, write_json=False, strict=False)
        self.assertTrue(r.success)
        self.assertIn("widgetShowWhen", r.html)


class TestCascadingFilters(unittest.TestCase):
    """depends_on + options_from validation."""

    def test_validates_depends_on_existing_filter(self):
        from echart_dashboard import validate_manifest
        import pandas as pd
        m = {
            "schema_version": 1, "id": "cf", "title": "cf",
            "datasets": {"u": pd.DataFrame({"region": ["US"], "country": ["US"]})},
            "filters": [
                {"id": "country", "type": "select", "options": ["US"],
                  "depends_on": "nonexistent",
                  "options_from": {"dataset": "u", "key": "country"}},
            ],
            "layout": {"rows": [[]]}
        }
        ok, errs = validate_manifest(m)
        self.assertFalse(ok)
        self.assertTrue(any("nonexistent" in e for e in errs), errs)

    def test_self_depends_rejected(self):
        from echart_dashboard import validate_manifest
        import pandas as pd
        m = {
            "schema_version": 1, "id": "cf", "title": "cf",
            "datasets": {"u": pd.DataFrame({"x": [1]})},
            "filters": [
                {"id": "country", "type": "select", "options": ["US"],
                  "depends_on": "country", "options_from": {
                      "dataset": "u", "key": "x"
                  }},
            ],
            "layout": {"rows": [[]]}
        ...
        ok, errs = validate_manifest(m)
        self.assertFalse(ok)
        self.assertTrue(any("cannot depend on itself" in e for e in errs), errs)


class TestPivotWidget(unittest.TestCase):
    """Pivot widget validation + render."""
    ...
    def _manifest(self, **overrides):
        ...
        base = {
            "schema_version": 1, "id": "pv", "title": "pv",
            "datasets": {"perf": df},
            "layout": {"rows": [
                [{"widget": "pivot", "id": "p", "w": 12,
                   "dataset_ref": "perf",
                   "row_dim_columns": ["sector"],
                   "col_dim_columns": ["country"],
                   "value_columns": ["ret"]}]
            ]}
        }
        for k, v in overrides.items():
            base["layout"]["rows"][0][0][k] = v
        return base

    def test_basic_pivot_compiles(self):
        from echart_dashboard import compile_dashboard
        r = compile_dashboard(self._manifest(),
                                 write_html=False, write_json=False,
                                 strict=True)
        self.assertTrue(r.success)
        self.assertIn("pivot-p", r.html)
        self.assertIn("pivot-controls-p", r.html)

    def test_missing_dim_columns_rejected(self):
        from echart_dashboard import validate_manifest
        m = self._manifest()
        del m["layout"]["rows"][0][0]["row_dim_columns"]
        ok, errs = validate_manifest(m)
        self.assertFalse(ok)
        self.assertTrue(any("row_dim_columns" in e for e in errs))

    def test_color_scale_string_value(self):
        from echart_dashboard import validate_manifest
        m = self._manifest(color_scale="diverging")
        ok, errs = validate_manifest(m)
        self.assertTrue(ok, errs)


class TestInitialState(unittest.TestCase):
    """Per-widget initial_state pass-through into PAYLOAD."""

    def test_chart_initial_state_in_payload(self):
        from echart_dashboard import compile_dashboard
        import pandas as pd
        df = pd.DataFrame({"date": pd.date_range("2026-01-01", periods=5),
                              "x": [1, 2, 3, 4, 5]})
        m = {
            "schema_version": 1, "id": "is", "title": "is",
            "datasets": {"d": df},
            "layout": {"rows": [
                [{"widget": "chart", "id": "c", "w": 12,
                   "spec": {"chart_type": "line", "dataset": "d",
        ...
        self.assertTrue(r.success)
        self.assertIn("yoy_pct", r.html)


class TestStatStripScaffolding(unittest.TestCase):
    """The stat strip is opened via a per-tile toolbar button (Sigma
    glyph) and rendered into the shared popup modal. It is no longer
    emitted inline above each chart -- that was too cluttering once
    multiple time-series tiles stacked on a page. These tests pin the
    server-side scaffolding (toolbar button presence + suppression),
    not the JS modal mechanics."""
    ...
    # The button HTML carries the `stat-strip-btn` class. We anchor
    # assertions on that class rather than `data-stat-strip-toggle`
    # because the JS click handler also references the data attribute
    # in its body, which would false-positive a substring search.
    _BUTTON_MARKER = '"tile-btn stat-strip-btn"'

    def _has_strip_button(self, html: str) -> bool:
        return self._BUTTON_MARKER in html

    def test_strip_button_emitted_for_line(self):
        from echart_dashboard import compile_dashboard
        m = {
            "schema_version": 1, "id": "ss", "title": "ss",
            "datasets": {"d": self._df()},
            "layout": {"rows": [
                [{"widget": "chart", "id": "c", "w": 12,
                   "spec": {"chart_type": "line", "dataset": "d",
        ...
        r = compile_dashboard(m, write_html=False, write_json=False, strict=True)
        self.assertTrue(r.success)
        # Toolbar Sigma button present, but no inline strip container.
        self.assertTrue(self._has_strip_button(r.html))
        self.assertNotIn('id="stat-strip-c"', r.html)

    def test_strip_suppressed_via_spec_flag(self):
        from echart_dashboard import compile_dashboard
        m = {
            "schema_version": 1, "id": "ss", "title": "ss",
            "datasets": {"d": self._df()},
        ...
        r = compile_dashboard(m, write_html=False, write_json=False, strict=True)
        self.assertTrue(r.success)
        # No toolbar button when stat_strip is suppressed at the spec.
        self.assertFalse(self._has_strip_button(r.html))

    def test_strip_button_suppressed_for_unsupported_chart_type(self):
        """Stat strip only renders for line / multi_line / area;
        a bar chart does not get a Sigma toolbar button even though
        bar isn't explicitly opted-out."""
        from echart_dashboard import compile_dashboard
        m = {
            "schema_version": 1, "id": "ss", "title": "ss",
            "datasets": {"d": self._df()},
...
class TestInCellTableViz(unittest.TestCase):
    """Table column in_cell: bar / sparkline validation."""
    ...
    def test_in_cell_bar_validates(self):
        from echart_dashboard import compile_dashboard
        m = {
            "schema_version": 1, "id": "ic", "title": "ic",
            "datasets": {"d": self._df()},
            "layout": {"rows": [
                [{"widget": "table", "id": "t", "w": 12,
                   "dataset_ref": "d",
    ...
    def test_in_cell_sparkline_requires_from_dataset(self):
        from echart_dashboard import validate_manifest
        m = {
            "schema_version": 1, "id": "ic", "title": "ic",
            "datasets": {"d": self._df()},
            "layout": {"rows": [
                [{"widget": "table", "id": "t", "w": 12,
                   ...
                   "columns": [
                       {"field": "ticker"},
                       {"field": "ret", "in_cell": "sparkline"}
                   ]}]
            ]}
        }
        ok, errs = validate_manifest(m)
        self.assertFalse(ok)
        self.assertTrue(any("from_dataset" in e or "row_key" in e
                                for e in errs), errs)
