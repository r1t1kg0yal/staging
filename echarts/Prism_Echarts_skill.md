...
| 5 | `Download Dashboard` | always (full view as single PNG) |
...
**Per-spec overrides** (theme/palette/styling). The chart `spec` may include `palette`, `theme`, `subtitle`, `title`, `annotations` to override manifest-level defaults for that one widget. Required keys: `chart_type`, `dataset`, `mapping`.
...
`spec.dataset` references `manifest.datasets.<name>`. At compile time the source rows are materialized into a pandas DataFrame and fed into the per-chart-type builder. `spec.chart_type` must be one of the 26 supported types (Section 3.1). `ref` paths resolve relative to (1) `base_dir` argument to `compile_dashboard`, (2) the loaded manifest file's parent directory when `compile_dashboard(path)` is called with a path, (3) current working directory.

**`option` variant for hand-crafted ECharts.** Use when none of the 26 builder types fit. The widget passes the raw option dict straight to ECharts; the compiler injects only theme + dimensions + dataZoom (unless suppressed).

**Widget presentation knobs (apply to every tile type):**

| Field | Purpose |
|-------|---------|
| `title` | Card header text. When set on a chart, the internal ECharts title is auto-suppressed (set `spec.keep_title: True` to opt out). |
...
| Download PNGs | always; one 2x PNG per chart, titles/subtitles baked in |
| Download Dashboard | always; full view as single 2x PNG via `html2canvas@1.4.1` (CDN, lazy-loaded). Filename `<id>_YYYY-MM-DD-HH-MM-SS.png` |
