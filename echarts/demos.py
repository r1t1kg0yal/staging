...
def build_rates_monitor(out_dir: Path) -> Dict[str, Any]:
    ...
    # Per-column field_provenance unlocks the auto stat strip
    # (compiler reads units to pick bp / pct+abs / arith deltas) and
    # populates the popup Sources footer with the upstream symbol +
    # source label. PRISM is expected to clean its data-pull metadata
    # into this shape.
    rates_prov = {
        "us_2y":  {"system": "market_data",
                     "symbol": "IR_USD_Treasury_2Y_Rate",
                     "units": "percent",
                     "display_name": "US 2Y",
                     "source_label": "FRED H.15"},
        "us_5y":  {"system": "market_data",
                     "symbol": "IR_USD_Treasury_5Y_Rate",
                     "units": "percent",
                     "display_name": "US 5Y",
                     "source_label": "FRED H.15"},
        "us_10y": {"system": "market_data",
                     "symbol": "IR_USD_Treasury_10Y_Rate",
                     "units": "percent",
                     "display_name": "US 10Y",
                     "source_label": "FRED H.15"},
        "us_30y": {"system": "market_data",
                     "symbol": "IR_USD_Treasury_30Y_Rate",
                     "units": "percent",
                     "display_name": "US 30Y",
                     "source_label": "FRED H.15"},
        "10y_real": {"system": "market_data",
                       "symbol": "IR_USD_Treasury_Real_10Y_Rate",
                       "units": "percent",
                       "display_name": "US 10Y Real",
                       "source_label": "FRED H.15"},
        "2s10s":  {"system": "computed",
                     "recipe": "(us_10y - us_2y) * 100",
                     "units": "bp",
                     "display_name": "2s10s",
                     "source_label": "Computed"},
        "5s30s":  {"system": "computed",
                     "recipe": "(us_30y - us_5y) * 100",
                     "units": "bp",
                     "display_name": "5s30s",
                     "source_label": "Computed"},
    }

    # Manifest-level computed dataset: rolling 60-day z-scores derived
    # from the source rates panel. The compiler evaluates the AST-whitelisted
    # recipe at compile time and auto-stamps system=computed provenance
    # with inferred units (z from `zscore(...)`).
    rates_compute = {
        "us_10y_z_60":  "zscore(us_10y, 60)",
        # 10y_real is computed inline (us_10y - 2.2) so we re-derive it
        # here rather than referencing the column whose name starts with
        # a digit (which the safe-eval parser rejects).
        "real_10y_z_60": "zscore(us_10y - 2.2, 60)",
    }

    datasets = {
        "rates": {"source": df_rates, "field_provenance": rates_prov},
        "rates_derived": {"from": "rates", "compute": rates_compute},
        "ism_monthly": df_ism,
        "cb_rates": df_cb,
    }
        ...
        "layout": {"kind": "tabs", "tabs": [
            {"id": "overview", "label": "US overview",
              "description": ("US Treasury curve and 2s10s spread with "
                               "brush cross-filter."),
              "rows": [
                # show_when (data condition): the inversion warning only
                # renders when the latest 2s10s spread is below 0.
                [
                    {"widget": "note", "id": "n_inversion", "w": 12,
                      "kind": "risk", "title": "2s10s inverted",
                      "body": ("Curve flipped negative on the latest tick. "
                                "Front-end stress historically front-runs "
                                "policy easing -- watch the FOMC tape into "
                                "next week."),
                      "show_when": {"data": "rates.latest.2s10s < 0"}},
                ],
                [
            ...
            {"id": "macro", "label": "US macro overlay",
              "description": ("ISM Manufacturing PMI with expansion / "
                               ...
                               "plus a 10Y YoY change chart that opens "
                               "in the YoY % transform via initial_state."),
              "rows": [
                [
                ...
                [
                    # initial_state: open this chart already in YoY
                    # change mode + 5-period smoothing. The drawer
                    # state is seeded from spec.initial_state on init,
                    # so the first paint shows the transform applied.
                    {"widget": "chart", "id": "us10y_yoy",
                      "w": 12, "h_px": 380,
                      "title": "US 10Y, YoY change (initial_state demo)",
                      "subtitle": ("spec.initial_state opens the chart in "
                                     "YoY change mode with 5-period "
                                     "smoothing."),
                      "spec": {
                          "chart_type": "line", "dataset": "rates",
                ...
                [
                    # Computed-dataset chart: rolling 60d z-score of
                    # the 10Y. The rates_derived dataset is a
                    # `from: "rates", compute: {us_10y_z_60: "..."}`
                    # block; provenance is auto-stamped as
                    # system=computed with units=z.
                    {"widget": "chart", "id": "us10y_z",
                      "w": 12, "h_px": 380,
                      "title": "US 10Y, 60d rolling z-score (computed dataset)",
                      "subtitle": ("`rates_derived.compute.us_10y_z_60 = "
                                     "zscore(us_10y, 60)`; provenance "
                                     "auto-stamped as system=computed."),
                      "spec": {
                          "chart_type": "line", "dataset": "rates_derived",
                          "mapping": {"x": "date", "y": "us_10y_z_60",
                                        "y_title": "z (60d rolling)"},
                          "annotations": [
                              {"type": "hline", "y": 2,
                                "label": "+2σ", "style": "dashed"},
                              {"type": "hline", "y": -2,
                                "label": "-2σ", "style": "dashed"},
                          ]}},
                ],
...
def build_risk_regime(out_dir: Path) -> Dict[str, Any]:
    ...
    manifest = {
        ...
        "layout": {"rows": [
                ...
                {"widget": "chart", "id": "corr", "w": 6, "h_px": 420,
                  ...
                  "spec": {
                      ...
                      "mapping": {
                          ...
                          "value_decimals": 2}}},
                {"widget": "chart", "id": "vix",
                "title": 'VIX term structure by regime', "w": 6, "h_px": 420,
                  "spec": {
                  ...
                  "spec": {
                      ...
                      "annotations": [
                          ...
                          {"type": "vline", "x": 0, "color": "#aaa",
                            "style": "dashed"}]}},
                {"widget": "chart", "id": "factors",
                "title": 'Factor returns (last 5Y)', "w": 6, "h_px": 380,
                  "spec": {
            ...
            [
                {"widget": "chart", "id": "drawdown",
                "subtitle": 'Band marks the bear-market zone; arrow points from the trough to the recovery.',
                "title": 'Running drawdown', "w": 12, "h_px": 360,
                  "spec": {
...
def _pull_fomc_rate_path_fan() -> pd.DataFrame:
    """Synthetic Fed funds rate-path forecast: a central path plus
    market-implied bounds at +/-1 sigma and +/-2 sigma. Used for the
    fan_cone forecast chart."""
    random.seed(SEED + 105)
    dates = pd.date_range(start=END_DATE, periods=24, freq="MS")
    ffr = 5.375
    rows = []
    for i, d in enumerate(dates):
        # Central path: cuts to ~3.0 over 24 months
        target = 5.375 - (5.375 - 3.0) * (i / max(1, len(dates) - 1))
        ffr = ffr + (target - ffr) * 0.30 + random.gauss(0, 0.04)
        # Bands widen with horizon: 1sigma ~ 0.04 * sqrt(months),
        # 2sigma double that.
        sigma = 0.05 * (i + 1) ** 0.5
        rows.append({
            "date": d,
            "central_pct": round(ffr, 3),
            "p25_pct":     round(ffr - 1.0 * sigma, 3),
            "p75_pct":     round(ffr + 1.0 * sigma, 3),
            "p10_pct":     round(ffr - 2.0 * sigma, 3),
            "p90_pct":     round(ffr + 2.0 * sigma, 3),
        })
    return pd.DataFrame(rows)
...
def build_fomc_monitor(out_dir: Path) -> Dict[str, Any]:
    ...
    df_path_fan = _pull_fomc_rate_path_fan()
            ...
            {"id": "dots", "label": "Dots & path",
              ...
              "rows": [
                [
                    ...
                    "subtitle": 'solid = realized / market-implied, dashed = SEP median',
                ...
                [
                    {"widget": "chart", "id": "rate_path_fan",
                      "w": 12, "h_px": 380,
                      "title": ("Fed funds rate-path fan "
                                  "(market-implied + 50% / 80% bands)"),
                      "subtitle": ("Central path + two confidence "
                                     "ribbons (25-75% and 10-90%) "
                                     "rendered as a fan_cone."),
                      "spec": {
                          "chart_type": "fan_cone",
                          "dataset": "path_fan",
                          "mapping": {
                              "x": "date", "y": "central_pct",
                              "y_title": "Fed funds (%)",
                              "bands": [
                                  {"lower": "p10_pct", "upper": "p90_pct",
                                    "label": "10-90% band",
                                    "opacity": 0.10},
                                  {"lower": "p25_pct", "upper": "p75_pct",
                                    "label": "25-75% band",
                                    "opacity": 0.25},
                              ]},
                          "annotations": [
                              {"type": "hline", "y": 2.0,
                                "label": "Neutral", "style": "dashed",
                                "color": "#999"},
                          ]}},
                ],
                [
            ...
            {"id": "calendar", "label": "Decision calendar",
              ...
              "rows": [
                [
                    ...
                    "title": '2025 FOMC decision calendar (bp change)',
                      "w": 12, "h_px": 260,
                      "spec": {
            ...
            {"id": "network", "label": "Counterparty network",
              ...
              "rows": [
                [
                    ...
                    "title": 'Counterparty exposure network (drag nodes)',
                      "w": 8, "h_px": 520,
                      "spec": {
                    ...
                    "subtitle": 'Org-style view: tier -> firm. Click a node to collapse.',
            ...
            {"id": "funnel", "label": "Mandate funnel",
              ...
              "rows": [
                [
                    ...
                    "title": 'Prospect-to-funded pipeline (USD bn)',
                      "w": 8, "h_px": 440,
                      "spec": {
            ...
            {"id": "fundamentals", "label": "Fundamentals",
              ...
              "rows": [
                [
                    ...
                    "subtitle": 'solid = actual, dashed = consensus',
                    "title": 'Quarterly EPS: actuals vs consensus',
                      "w": 8, "h_px": 380,
                      "spec": {
            ...
            {"id": "risk", "label": "Risk & valuation",
              ...
              "rows": [
                ...
                [
                    ...
                    "title": 'Valuation vs 5Y range (z-score colored)',
                      "w": 12, "h_px": 360,
                      "spec": {
...
def build_portfolio_analytics(out_dir: Path) -> Dict[str, Any]:
    ...
    # Waterfall: synthetic MTD P&L attribution (one row per
    # contribution step, with Open / Close marked as full-height
    # totals). Reads cleanly as a P&L bridge.
    df_wf = pd.DataFrame([
        {"step": "Open MTD",      "delta":   0.0, "is_total": True},
        {"step": "Equity beta",   "delta":  3.4, "is_total": False},
        {"step": "Sector tilt",   "delta":  1.1, "is_total": False},
        {"step": "Single-name",   "delta":  0.6, "is_total": False},
        {"step": "Rates carry",   "delta":  0.5, "is_total": False},
        {"step": "Curve",         "delta": -0.4, "is_total": False},
        {"step": "FX hedge",      "delta": -0.2, "is_total": False},
        {"step": "Credit spread", "delta":  0.3, "is_total": False},
        {"step": "Costs / fees",  "delta": -0.1, "is_total": False},
        {"step": "Close MTD",     "delta":  5.2, "is_total": True},
    ])

    # Marimekko: 2D allocation (bucket x size). Each cell's area is
    # share-of-book in percentage points.
    df_mkko = pd.DataFrame([
        {"bucket": "Equities",    "size_band": "Mega",  "share": 22.0},
        {"bucket": "Equities",    "size_band": "Large", "share": 12.0},
        {"bucket": "Equities",    "size_band": "Mid",   "share":  6.0},
        {"bucket": "Rates",       "size_band": "Mega",  "share": 14.0},
        {"bucket": "Rates",       "size_band": "Large", "share":  6.0},
        {"bucket": "Credit",      "size_band": "Mega",  "share":  9.0},
        {"bucket": "Credit",      "size_band": "Large", "share":  4.0},
        {"bucket": "Credit",      "size_band": "Mid",   "share":  2.0},
        {"bucket": "Commodities", "size_band": "Mega",  "share":  6.0},
        {"bucket": "Commodities", "size_band": "Mid",   "share":  3.0},
        {"bucket": "FX",          "size_band": "Mega",  "share":  5.0},
        {"bucket": "Cash",        "size_band": "Mega",  "share": 11.0},
    ])

    # Pivot: long-form returns by (book, region, window) so the
    # viewer can repivot at runtime via the dropdowns.
    pivot_rows: List[Dict[str, Any]] = []
    rng = random.Random(SEED + 600)
    for book in ["Macro", "Equity LS", "Credit", "Vol", "Commod"]:
        for region in ["US", "EU", "AP", "EM"]:
            for window in ["1d", "5d", "MTD", "YTD"]:
                pivot_rows.append({
                    "book": book, "region": region, "window": window,
                    "ret_pct": round(rng.gauss(0.4, 1.2), 2),
                })
    ...
    manifest = {
        ...
        "layout": {"kind": "grid", "cols": 12, "rows": [
            ...
            [{"widget": "divider", "id": "div_attribution"}],
            [
                {"widget": "markdown", "id": "h_attr", "w": 12,
                  "content": "## Attribution & allocation"},
            ],
            [
                {"widget": "chart", "id": "wf_attribution",
                  "w": 6, "h_px": 380,
                  "title": "MTD P&L bridge (waterfall)",
                  "subtitle": ("Open / Close drawn as full-height "
                                 "totals; intermediate steps are "
                                 "signed deltas (green up / red down)."),
                  "spec": {
                      "chart_type": "waterfall", "dataset": "wf",
                      "mapping": {"x": "step", "y": "delta",
                                    "is_total": "is_total",
                                    "y_title": "P&L ($mm)"}}},
                {"widget": "chart", "id": "alloc_mkko",
                  "w": 6, "h_px": 380,
                  "title": "Bucket x size allocation (marimekko)",
                  "subtitle": ("Each column's width is its share of "
                                 "book; cell heights are size-band "
                                 "shares within the bucket."),
                  "spec": {
                      "chart_type": "marimekko", "dataset": "mkko",
                      "mapping": {"x": "bucket", "y": "size_band",
                                    "value": "share"}}},
            ],
            [
                {"widget": "pivot", "id": "perf_pivot", "w": 12,
                  "title": "Returns by book / region / window (pivot)",
                  "subtitle": ("Long-form returns dataset; pick the "
                                 "row dim, column dim, value, and "
                                 "aggregator with the dropdowns."),
                  "dataset_ref": "perf_long",
                  "row_dim_columns": ["book", "region"],
                  "col_dim_columns": ["window"],
                  "value_columns":   ["ret_pct"],
                  "agg_options":     ["mean", "median", "sum",
                                        "min", "max", "count"],
                  "row_default": "book", "col_default": "window",
                  "value_default": "ret_pct", "agg_default": "mean",
                  "decimals": 2,
...
def _pull_wrap_movers_history(tickers: List[str], days: int = 60
                                  ) -> pd.DataFrame:
    """Synthetic 60d daily price history for each top-mover ticker.

    Used by the in-cell sparkline column on the top-movers table -- one
    row per (ticker, date) so the table renderer can filter to a row's
    own ticker and draw an inline 80x16 sparkline.
    """
    random.seed(SEED + 410)
    dates = pd.date_range(end=END_DATE, periods=days, freq="B")
    rows: List[Dict[str, Any]] = []
    for t in tickers:
        # Random walk anchored to ~100 with per-ticker drift, lifted
        # so values stay positive
        price = 100.0
        drift = random.gauss(0, 0.04)
        for d in dates:
            price = max(2.0, price * (1 + drift / 100
...
def build_markets_wrap(out_dir: Path) -> Dict[str, Any]:
    ...
    df_mov_hist = _pull_wrap_movers_history(
        df_mov["ticker"].tolist(), days=60
    )
    # Long-form snapshot dataset for the slope chart: one row per
    # (sector, snapshot) where snapshot is "MTD" / "YTD". The slope
    # builder reads two distinct x values and joins each sector's
    # left + right with a sloped line.
    df_sec_slope = pd.DataFrame([
        {"sector": r["sector"], "snapshot": "MTD", "ret": r["mtd_pct"]}
        for r in _pull_wrap_sectors().to_dict("records")
    ] + [
        {"sector": r["sector"], "snapshot": "YTD", "ret": r["ytd_pct"]}
        for r in _pull_wrap_sectors().to_dict("records")
    ])
    ...
    manifest = {
        ...
        "datasets": {
            ...
            "mov": df_mov, "mov_hist": df_mov_hist,
            "sec_slope": df_sec_slope, "chg": df_chg,
        },
        "filters": [
        ...
        "layout": {"kind": "grid", "cols": 12, "rows": [
            ...
            [
                {"widget": "chart", "id": "eq_slope",
                  "w": 12, "h_px": 360,
                  "title": "S&P 500 sectors: MTD vs YTD (slope chart)",
                  "subtitle": ("Per-sector lines connect month-to-date "
                                 "to year-to-date so you can read direction "
                                 "+ rotation in one frame."),
                  "spec": {
                      "chart_type": "slope",
                      "dataset": "sec_slope",
                      "mapping": {"x": "snapshot", "y": "ret",
                                    "color": "sector",
                                    "y_title": "Return (%)"}}},
            ],
            ...
            [
                {"widget": "table", "id": "movers_table", "w": 12,
                  "dataset_ref": "mov",
                  "title": ("Single-name top movers (in-cell bars + "
                              "sparkline)"),
                  "subtitle": ("1D / MTD / YTD columns render with "
                                 "in-cell bars; the rightmost column is "
                                 "a 60d sparkline read from `mov_hist`."),
                  "searchable": True, "sortable": True,
                  "max_rows": 20, "row_height": "compact",
                  "info": ("Rows highlighted green / red based on "
                             "1-day move. Numeric columns use in_cell=bar "
                             "for visual scanning; the sparkline column "
                             "uses in_cell=sparkline + from_dataset."),
                  "row_highlight": [
                      {"field": "d1_pct", "op": ">",
                        "value": 2.0, "class": "pos"},
                      {"field": "d1_pct", "op": "<",
                  ...
                  "columns": [
                      ...
                      {"field": "ticker", "label": "Spark (60d)",
                        "in_cell": "sparkline",
                        "from_dataset": "mov_hist",
                        "row_key": "ticker",
                        "filter_field": "ticker",
                        "value": "price",
                        "show_text": False, "align": "left"},
                  ]},
            ],
        ]},
        "links": [
    ...
    manifest = {
        ...
        "filters": [
            ...
            {"id": "bs_sector", "type": "select", "default": "",
              ...
              "field": "sector", "label": "Sector",
              "targets": ["bond_table"]},
            # Cascading filter: bs_issuer's options are rebuilt from
            # the bond_main dataset whenever the upstream sector changes.
            # ${bs_sector} substitutes the live filterState value.
            {"id": "bs_issuer", "type": "select", "default": "",
              "all_value": "",
              "options": [""],
              "depends_on": "bs_sector",
              "options_from": {"dataset": "bonds",
                                 "key": "issuer",
                                 "where": "sector == ${bs_sector}"},
              "field": "issuer", "label": "Issuer (cascades from sector)",
              "targets": ["bond_table"]},
              ...
              "rows": [
                ...
                [
                    {"widget": "table", "id": "rv_table", "w": 12,
                      "dataset_ref": "rv",
                      "title": ("RV screen (sorted by |z| desc via "
                                  "initial_state)"),
                      "searchable": True, "sortable": True,
                      "max_rows": 50,
                      # initial_state seeds the controls drawer state
                      # so the table opens sorted on `z` desc with
                      # 2-decimal numeric formatting.
                      "initial_state": {
                          "sort_by": "z", "sort_dir": "desc",
                          "decimals": 2,
                      },
...
def build_feature_studio(out_dir: Path) -> Dict[str, Any]:
    """End-to-end exercise for the new manifest features:

      * 4 new chart types: waterfall, slope, fan_cone, marimekko
      * Computed datasets (manifest-level expressions)
      * show_when (compile-time data condition + runtime filter)
      * Cascading filters (depends_on + options_from)
      * In-cell data bars + sparklines in tables
      * Pivot widget
      * Per-widget initial_state (chart opens in YoY %)
      * Auto-computed stat strip (free; no per-spec config)

    Synthetic finance-flavoured data so the demo reads cleanly without
    depending on any external pulls.
    """
    ...
    # ---- core time series ------------------------------------------------
    n = 250
    dates = pd.date_range(today - pd.Timedelta(days=n - 1), periods=n,
                            freq="B")
    df_rates = pd.DataFrame({
        "date": dates,
        "us_2y":  4.10 + np.cumsum(rng.standard_normal(n)) * 0.012,
        "us_5y":  4.20 + np.cumsum(rng.standard_normal(n)) * 0.015,
        "us_10y": 4.30 + np.cumsum(rng.standard_normal(n)) * 0.018,
        "us_30y": 4.55 + np.cumsum(rng.standard_normal(n)) * 0.020,
    })
    df_rates["vix"] = 14 + np.abs(np.cumsum(rng.standard_normal(n)) * 0.4)

    # ---- waterfall: synthetic P&L attribution ----------------------------
    df_pnl = pd.DataFrame({
        "step":     ["Open", "Carry", "Roll", "Curve",
                      "Spread", "Fees", "Close"],
        "delta":    [100.0, 6.2, -2.4, 8.5, -1.8, -0.9, 109.6],
        "is_total": [True, False, False, False, False, False, True],
    })

    # ---- slope: month-end vs latest sector returns -----------------------
    df_slope = pd.DataFrame({
        "snapshot": ["Mar 31", "Apr 25"] * 5,
        "sector": ["Tech", "Tech", "Health", "Health",
                     "Energy", "Energy", "Financials", "Financials",
                     "Utilities", "Utilities"],
        "ret":      [12.0, 18.4, 4.5, -2.1, -3.0, -8.7,
                      6.0, 8.2, 1.0, 0.8],
    })

    # ---- fan cone: 10Y forecast ribbon -----------------------------------
    fcst_n = 60
    fcst_dates = pd.date_range(today, periods=fcst_n, freq="B")
    central = np.linspace(4.07, 4.40, fcst_n)
    df_fan = pd.DataFrame({
        "date":    fcst_dates,
        "central": central,
        "p10":     central - 0.45,
    ...
    # ---- marimekko: cap-weighted sector x size mosaic --------------------
    df_mkko = pd.DataFrame({
        "sector":      (["Tech"] * 3 + ["Health"] * 3
                         + ["Energy"] * 3 + ["Financials"] * 3),
        "size_bucket": (["Mega", "Large", "Mid"] * 4),
        "share":       [42.0, 18.0, 6.0,
                          14.0,  9.0, 3.0,
                           4.0,  2.5, 1.5,
                          18.0, 10.0, 4.0],
    ...
    # ---- top-movers table (in-cell bar + sparkline) ----------------------
    movers_universe = [
        ("AAPL", "Tech",        "US"),
        ("MSFT", "Tech",        "US"),
        ("NVDA", "Tech",        "US"),
        ("TSLA", "Discretionary","US"),
        ("XOM",  "Energy",      "US"),
        ("CVX",  "Energy",      "US"),
        ("JPM",  "Financials",  "US"),
        ("BAC",  "Financials",  "US"),
        ("ASML", "Tech",        "EU"),
        ("LVMH", "Discretionary","EU"),
        ("TM",   "Discretionary","JP"),
    ]
    df_top = pd.DataFrame({
        "ticker": [t[0] for t in movers_universe],
        "sector": [t[1] for t in movers_universe],
        "country":[t[2] for t in movers_universe],
        "ret":    rng.standard_normal(len(movers_universe)) * 1.6,
        "ytd":    rng.standard_normal(len(movers_universe)) * 8 + 5,
    })

    # Sparkline lookup dataset: 60 rows of synthetic price history per ticker
    spark_rows = []
    for t, *_ in movers_universe:
        path = 100 + np.cumsum(rng.standard_normal(60) * 0.8)
        for i, p in enumerate(path):
            spark_rows.append({
                "date": dates[-60 + i].strftime("%Y-%m-%d"),
                "ticker": t,
                "price": float(p),
            })
    df_spark = pd.DataFrame(spark_rows)

    # ---- pivot: long-form perf data --------------------------------------
    pivot_rows = []
    for t in movers_universe:
        for win in ["1d", "5d", "MTD", "YTD"]:
            pivot_rows.append({
                "ticker": t[0],
                "sector": t[1],
                "country": t[2],
                "window": win,
                "ret": rng.standard_normal() * 1.5,
            })
    df_pivot = pd.DataFrame(pivot_rows)

    # ---- universe for cascading filters ----------------------------------
    df_universe = pd.DataFrame({
        "region":  ["NA", "NA", "EU", "EU", "EU", "AP", "AP"],
        "country": ["US", "Canada", "UK", "Germany", "France", "Japan", "Australia"],
    })

    manifest: Dict[str, Any] = {
        "schema_version": 1,
        "id": "feature_studio",
        "title": "Feature studio (new bells & whistles)",
        "description": ("End-to-end exercise of the new manifest features: "
                          "computed datasets, show_when, cascading filters, "
                          "in-cell bars / sparklines, pivot widget, four new "
                          "chart types (waterfall / slope / fan_cone / "
                          "marimekko), per-widget initial_state, and the "
                          "auto-computed stat strip."),
        "theme": "gs_clean", "palette": "gs_primary",
        "metadata": {
            "data_as_of": today.strftime("%Y-%m-%dT16:00:00Z"),
            "generated_at": datetime.now(timezone.utc).isoformat(),
            "sources": ["Synthetic"],
            "tags": ["features", "demo"],
            "summary": (
                "Each tab below isolates one feature. The header strips, "
                "filters, and pivot all wire up automatically. Click a "
                "drawer **(\u22EE)** on any chart to flip transforms / "
                "smoothing -- the URL hash updates so a colleague gets "
                "your exact view by sharing the link."
            ),
        },
        "datasets": {
            "rates": {"source": df_rates,
                       "field_provenance": {
                           "us_2y":  {"system": "market_data",
                                       "symbol": "IR_USD_Treasury_2Y_Rate",
                                       "units": "percent",
                           ...
                           "us_30y": {"system": "market_data",
                                       ...
                                       "units": "percent",
                                       "display_name": "US 30Y"},
                           "vix":    {"system": "market_data",
                                       "symbol": "VOL_VIX_Index",
                                       "units": "index",
                                       "display_name": "VIX"},
                       }},
            "spreads": {
        ...
        "layout": {"kind": "tabs", "tabs": [
            {"id": "core", "label": "Stat strip + initial state",
              "description": ("Two charts: bare line (auto stat strip; "
                                "fed by units in field_provenance) and the "
                                "same line opening in YoY % via "
                                "spec.initial_state."),
              "rows": [
                  [{"widget": "chart", "id": "ust_levels",
                     "w": 6, "h_px": 360,
                     "title": "UST 10Y (levels)",
                     "subtitle": "Stat strip auto-computes from units = percent.",
                     "spec": {"chart_type": "line", "dataset": "rates",
                                "mapping": {"x": "date", "y": "us_10y",
                                             "y_title": "Yield (%)"}}},
                   {"widget": "chart", "id": "ust_yoy",
                     "w": 6, "h_px": 360,
                     "title": "UST 10Y (YoY % via initial_state)",
                     "subtitle": "Same dataset; spec.initial_state opens in YoY %.",
                     "spec": {"chart_type": "line", "dataset": "rates",
                                "mapping": {"x": "date", "y": "us_10y",
                                             "y_title": "Yield"},
                                "initial_state": {"transform": "yoy_pct",
                                                    "smoothing": 5}}}],
                  [{"widget": "chart", "id": "spread_z", "w": 12, "h_px": 320,
                     "title": "Computed dataset: 2s10s spread (bp) + 60d z",
                     "subtitle": ("`spreads.compute.us_2s10s_bp = "
                                    "(us_10y - us_2y) * 100`; provenance "
                                    "auto-stamped as system=computed."),
                     "spec": {"chart_type": "multi_line",
                                "dataset": "spreads",
                                "mapping": {"x": "date",
            ...
            {"id": "shape", "label": "New chart types",
              "description": ("Four finance-flavoured types added to the "
                                "builder catalogue: waterfall, slope, fan, "
                                "marimekko."),
              "rows": [
                  [{"widget": "chart", "id": "wf",
                     "w": 6, "h_px": 360,
                     "title": "P&L bridge (waterfall)",
                     "subtitle": "is_total flags Open / Close as full bars.",
                     "spec": {"chart_type": "waterfall",
                                "dataset": "pnl_bridge",
                                "mapping": {"x": "step", "y": "delta",
                                             "is_total": "is_total",
                                             "y_title": "Contribution (bp)"}}},
                   {"widget": "chart", "id": "slope",
                     "w": 6, "h_px": 360,
                     "title": "Sector returns: month-end vs latest (slope)",
                     "spec": {"chart_type": "slope", "dataset": "slope",
                                "mapping": {"x": "snapshot", "y": "ret",
                                             "color": "sector",
                                             "y_title": "Return (%)"}}}],
                  [{"widget": "chart", "id": "fan",
                     "w": 6, "h_px": 360,
                     "title": "10Y forecast cone",
                     "subtitle": "Two confidence ribbons (10-90% / 25-75%).",
                     "spec": {"chart_type": "fan_cone", "dataset": "fan",
                                "mapping": {
                                    "x": "date", "y": "central",
                                    "bands": [
                                        {"lower": "p10", "upper": "p90",
                                    ...
                                    "y_title": "Yield (%)"}}},
                   {"widget": "chart", "id": "mkko",
                     "w": 6, "h_px": 360,
                     "title": "Cap-weighted sector x size (marimekko)",
                     "spec": {"chart_type": "marimekko",
                                "dataset": "mkko",
                                "mapping": {"x": "sector",
                                             "y": "size_bucket",
                                             "value": "share"}}}],
              ]},
            {"id": "tables", "label": "Tables + pivot",
              "description": ("In-cell bar (movers) + in-cell sparkline "
                                "(price history). Pivot widget below. "
                                "show_when=risk note appears only when "
                                "VIX is elevated."),
              "rows": [
                  [{"widget": "note", "id": "vol_note",
                     "kind": "risk", "title": "Vol regime elevated",
                     "body": ("VIX latest is above 20 -- expect wider "
                                "two-way and tighter stops."),
                     "show_when": {"data": "rates.latest.vix > 20"}}],
                  [{"widget": "table", "id": "mvr",
                     "w": 12,
                     "title": "Top movers (in-cell bar + sparkline)",
                     "dataset_ref": "movers", "searchable": True,
                     "max_rows": 50,
                     "columns": [
                         {"field": "ticker", "label": "Ticker"},
                         {"field": "sector", "label": "Sector"},
                         {"field": "country", "label": "Country"},
                         {"field": "ret", "label": "1d ret",
                           "format": "signed:2",
                           "in_cell": "bar"},
                         {"field": "ytd", "label": "YTD %",
                           "format": "signed:1",
                  ...
                  [{"widget": "pivot", "id": "perf_pivot", "w": 12,
                     "title": "Sector x window pivot (drag the dropdowns)",
                     "subtitle": ("Long-form perf data; user picks row dim, "
                                    "col dim, value, agg."),
                     "dataset_ref": "perf_long",
            ...
            {"id": "filters", "label": "Cascading filters",
              "description": ("Region drives Country options. Pick "
                                "EU and the Country dropdown rebuilds "
                                "with UK / Germany / France."),
              "rows": [
                  [{"widget": "table", "id": "country_view",
                     "w": 12,
                     "title": "Universe (region -> country cascade)",
                     "dataset_ref": "universe",
                     "searchable": True,
                     "columns": [
                         {"field": "region", "label": "Region"},
                         {"field": "country", "label": "Country"},
    ...
    r = compile_dashboard(manifest, strict=True,
                            output_path=str(out_dir / "dashboard.html"))
    thumb = _thumbnail(r.html_path, out_dir / "thumbnail.png",
                        width=1500, height=1700)
    return _result(r, thumb)
...
DEMO_REGISTRY: Dict[str, Dict[str, Any]] = {
    "rates_monitor": {
        ...
        "features": ["stat_strip", "computed_datasets", "show_when",
                       "initial_state"],
    },
    "risk_regime": {
        "title": "Risk regime monitor",
    ...
    "fomc_monitor": {
        ...
        "description": ("Fed policy: probability gauge, FFR futures "
                         ...
                         "heatmap, dual-axis rates chart, plus a "
                         "fan_cone rate-path forecast on the Dots & "
                         "path tab. Metadata + header_actions."),
        "kind": "dashboard",
        "build": build_fomc_monitor,
        "features": ["fan_cone", "stat_strip"],
    },
    "global_flows": {
        "title": "Global flows & allocation",
    ...
    "portfolio_analytics": {
        ...
        "description": ("Multi-asset portfolio: pinned KPI ribbon, "
                         ...
                         "stacked-area P&L, parallel-coords positions, "
                         "return-distribution histogram, stat_grid, "
                         "lineY brush. The Attribution & allocation "
                         "section adds a waterfall P&L bridge, "
                         "marimekko bucket x size mosaic, and a "
                         "pivot widget over a long-form returns "
                         "panel."),
        "kind": "dashboard",
        "build": build_portfolio_analytics,
        "features": ["waterfall", "marimekko", "pivot", "stat_strip"],
    },
    "markets_wrap": {
        "title": "Cross-asset end-of-day wrap",
        "description": ("One big scrollable cross-asset wrap: image "
                         ...
                         "colour scale, in-cell bars on 1D / MTD / "
                         "YTD columns + 60d sparklines per row, "
                         "and a slope chart for sector MTD vs YTD. "
                         "Stress-tests layout with 17 charts. "
                         "Filter `fx_*` / `cm_*` wildcard prefixes "
                         "and `legend` sync across the asset-class "
                         "panels."),
        "kind": "dashboard",
        "build": build_markets_wrap,
        "features": ["in_cell_bar", "in_cell_sparkline", "slope",
                       "stat_strip"],
    },
    "screener_studio": {
        "title": "Screener studio (tables / filters / drill-down)",
        "description": ("Three-tab toolbox. Tab 1: RV screen with "
                         ...
                         "+ drill-down time-series + initial_state "
                         "opens the table sorted by z desc. Tab 2: "
                         "all 9 filter types on one dataset. Tab 3: "
                         "bond universe with row_click.detail rich-"
                         "modal (stats / markdown / charts / sub-"
                         "table sections) + a cascading filter pair "
                         "(sector -> issuer rebuilt at runtime)."),
        "kind": "dashboard",
        "build": build_screener_studio,
        "features": ["cascading_filters", "initial_state"],
    },
    "bond_carry_roll": {
    ...
    "feature_studio": {
        "title": "Feature studio (canonical reference)",
        "description": ("Single-page reference dashboard with EVERY "
                         "new manifest feature in isolation: computed "
                         "datasets (manifest-level expressions with "
                         "auto-provenance), show_when (compile + "
                         "runtime conditional widget visibility), "
                         "cascading filters (region -> country), "
                         "in-cell bars + sparklines in tables, the "
                         "pivot / crosstab widget, four new chart types "
                         "(waterfall / slope / fan_cone / marimekko), "
                         "per-widget initial_state, and the auto-"
                         "computed stat strip. The other demos in the "
                         "gallery exercise these features in their "
                         "natural production context."),
        "kind": "dashboard",
        "build": build_feature_studio,
        "features": ["stat_strip", "url_state", "computed_datasets",
                       "show_when", "cascading_filters", "in_cell_bar",
                       "in_cell_sparkline", "waterfall", "slope",
                       "fan_cone", "marimekko", "initial_state",
                       "pivot"],
    },
}
...
def _gallery_html(results: List[Dict[str, Any]], out_root: Path) -> str:
    ...
    # Build a feature -> [demos] coverage matrix to surface MECE
    # coverage at the top of the gallery. Pulled from the same
    # `features` lists the cards render below.
    _FEATURE_LABELS = {
        "stat_strip":         "stat strip (free)",
        "url_state":          "URL state (free)",
        "computed_datasets":  "computed datasets",
        "show_when":          "show_when",
        "cascading_filters":  "cascading filters",
        "in_cell_bar":        "in-cell bar",
        "in_cell_sparkline":  "in-cell sparkline",
        "waterfall":          "waterfall",
        "slope":              "slope",
        "fan_cone":           "fan_cone",
        "marimekko":          "marimekko",
    ...
    coverage: Dict[str, List[str]] = {k: [] for k in _FEATURE_LABELS}
    for r in results:
        for f in r.get("features", []) or []:
            if f in coverage:
                coverage[f].append(r.get("name", ""))
    # `url_state` runs on every dashboard, so list it as covered by
    # every successful demo automatically (rather than asking each
    # registry entry to declare it explicitly).
    for r in results:
        if r.get("success"):
            n = r.get("name", "")
            if n and n not in coverage["url_state"]:
                coverage["url_state"].append(n)
    coverage_rows = "\n".join(
        f'<tr><td class="ftname">{_FEATURE_LABELS[k]}</td>'
        f'<td class="ftdemos">{", ".join(sorted(coverage[k])) or "(none)"}</td></tr>'
        for k in _FEATURE_LABELS
    )
...
<style>
  ...
  .missing {{ background:#fff5f5; color:#c53030; padding:20px; text-align:center;
                font-size:12px; }}
  .feat-row {{ padding:10px 18px 0; display:flex; flex-wrap:wrap; gap:6px; }}
  .feat-chip {{ background:#ebf4ff; color:#1a365d; border:1px solid #bee3f8;
                 padding:2px 8px; border-radius:999px; font-size:10.5px;
                 font-weight:500; }}
  .feat-chip.muted {{ background:#edf2f7; color:#718096;
                        border-color:#e2e8f0; }}
  /* feature-coverage matrix at top of gallery */
  .coverage {{ background:white; border:1px solid #e2e8f0; border-radius:10px;
                 padding:14px 18px; margin-bottom:18px; }}
  .coverage h3 {{ font-size:13px; margin:0 0 10px 0; color:#1a365d;
                    font-weight:600; }}
  .coverage table {{ width:100%; border-collapse:collapse;
                       font-size:12px; }}
  .coverage td {{ padding:5px 12px 5px 0; vertical-align:top; }}
  .coverage td.ftname {{ font-weight:600; color:#2d3748;
                          white-space:nowrap; width:180px;
                          font-family: ui-monospace, "SF Mono", Consolas, monospace;
                          font-size:11.5px; }}
  .coverage td.ftdemos {{ color:#4a5568; line-height:1.5; }}
...
<div class="wrap">
  ...
  <div class="coverage">
    <h3>New-feature coverage matrix (which demos exercise which features)</h3>
...
_FEATURE_CHIP_LABELS = {
    "stat_strip":         "stat strip",
    "url_state":          "URL state",
    "computed_datasets":  "computed",
    "show_when":          "show_when",
    "cascading_filters":  "cascading filters",
    "in_cell_bar":        "in-cell bar",
    "in_cell_sparkline":  "in-cell spark",
    "waterfall":          "waterfall",
    "slope":              "slope",
    "fan_cone":           "fan_cone",
    "marimekko":          "marimekko",
...
def _card(r: Dict[str, Any]) -> str:
    ...
    if features:
        chips = "".join(
            f'<span class="feat-chip">{_FEATURE_CHIP_LABELS.get(f, f)}</span>'
            for f in features
        )
        feat_row = f'<div class="feat-row">{chips}</div>'
    else:
        feat_row = (
            '<div class="feat-row">'
            '<span class="feat-chip muted">no new features</span>'
            '</div>'
        )
...
def run(out_root: Path, names: List[str], do_open: bool = False) -> int:
    ...
    for name in names:
        ...
        try:
            ...
            r["name"] = name
            r["features"] = list(entry.get("features", []) or [])
