#!/usr/bin/env python3
"""
chart_studio v0.3 -- single-file interactive editor for PRISM-style
...
is passed (or wrap_interactive_prism() is called separately), chart_studio
wraps the altair chart into a self-contained HTML editor with:
...
LIBRARY USAGE
    ...
    from chart_studio import wrap_interactive, wrap_interactive_prism
...
CLI USAGE
    ...
    python chart_studio.py                      # interactive menu
    python chart_studio.py wrap spec.json       # wrap and save HTML
    python chart_studio.py wrap spec.json --open --theme gs_clean
    python chart_studio.py demo                 # generate sample HTML files
    python chart_studio.py demo --matrix        # every sample x every theme
    python chart_studio.py list themes
    python chart_studio.py list palettes
    python chart_studio.py list dimensions
    python chart_studio.py list knobs --chart-type line
    python chart_studio.py info spec.json
    python chart_studio.py test                 # run built-in smoke tests
...
UNIVERSAL_KNOBS: List[Dict[str, Any]] = [
    ...
    {"name": "width",    "label": "Width",      "type": "range",
     ...
     "path": "width", "group": "Dimensions", "essential": True},
    {"name": "height",   "label": "Height",     "type": "range",
     "min": 150, "max": 1200, "step": 10, "default": 350,
     "path": "height", "group": "Dimensions", "essential": True},
    ...
    {"name": "xLabelAngle", "label": "X label angle", "type": "range",
     ...
     "path": "config.axisX.labelAngle", "group": "X-Axis", "essential": True},
    {"name": "xTickCount", "label": "X tick count", "type": "range",
     "min": 2, "max": 20, "step": 1, "default": 6,
     "path": "config.axisX.tickCount", "group": "X-Axis"},
    ...
    {"name": "xGridShow", "label": "X grid", "type": "checkbox",
     "default": True,
     "path": "config.axisX.grid", "group": "X-Axis"},
    {"name": "xDomainShow", "label": "X axis line", "type": "checkbox",
     "default": True,
     "path": "config.axisX.domain", "group": "X-Axis"},
    {"name": "xTickShow", "label": "X ticks", "type": "checkbox",
     "default": True,
     "path": "config.axisX.ticks", "group": "X-Axis"},
    ...
    {"name": "yLabelAngle", "label": "Y label angle", "type": "range",
     "min": -90, "max": 90, "step": 5, "default": 0,
     "path": "config.axisY.labelAngle", "group": "Y-Axis", "essential": True},
    {"name": "yTickCount", "label": "Y tick count", "type": "range",
     "min": 2, "max": 20, "step": 1, "default": 6,
     "path": "config.axisY.tickCount", "group": "Y-Axis"},
    ...
    {"name": "yGridShow", "label": "Y grid", "type": "checkbox",
     "default": True,
     "path": "config.axisY.grid", "group": "Y-Axis"},
    {"name": "yDomainShow", "label": "Y axis line", "type": "checkbox",
     "default": True,
     "path": "config.axisY.domain", "group": "Y-Axis"},
    {"name": "yTickShow", "label": "Y ticks", "type": "checkbox",
     "default": True,
     "path": "config.axisY.ticks", "group": "Y-Axis"},
...
.knob { margin: 4px 0; display: grid; grid-template-columns: 150px 1fr 55px; gap: 6px; align-items: center; }
...
/* Tabs below chart */
.below-tabs { margin-top: 16px; border: 1px solid #000; }
.tab-bar { display: flex; border-bottom: 1px solid #000; background: #f0f0f0; }
...
.stats-table { border-collapse: collapse; font-size: 11px; font-family: monospace; margin-top: 6px; }
.stats-table th, .stats-table td { border: 1px solid #ccc; padding: 3px 8px; text-align: right; }
.stats-table th { background: #f0f0f0; text-align: center; font-weight: bold; }
...
<div class="layout" id="mainLayout">
  ...
  <div class="panel chart-panel" id="chartPanel">
    <div class="chart-toolbar">
      ...
      <button onclick="toggleFullscreen()" title="Hide controls and expand chart" id="fullscreenBtn">Fullscreen</button>
...
<!-- Tabs below chart: Data / Code / Metadata / Export / Raw -->
<div class="below-tabs">
  ...
  <div class="tab-content" id="tab-data">
    <div class="tab-toolbar">
      ...
      <button onclick="downloadDataCSV()">Download CSV</button>
      <button onclick="downloadDataTSV()">Download TSV</button>
      <button onclick="downloadDataJSON()">Download JSON</button>
      <button onclick="copyDataAsMarkdown()">Copy as Markdown</button>
    </div>
    <div id="dataTableContainer"></div>
    <h3>Summary statistics</h3>
    <div id="summaryStatsContainer"></div>
    <div id="perSeriesStatsContainer"></div>
  </div>
...
const APPLY_FUNCTIONS = {
  ...
  setXAxisTitle: (spec, value) => {
    walkEncoding(spec, "x", enc => {
      if (value) enc.title = value; else delete enc.title;
    });
  },
  ...
  setXAxisFormat: (spec, value) => {
    if (!value) { setPath(spec, "config.axisX.format", undefined); return; }
    setPath(spec, "config.axisX.format", value);
  },
  setYAxisFormat: (spec, value) => {
    if (!value) { setPath(spec, "config.axisY.format", undefined); return; }
    setPath(spec, "config.axisY.format", value);
  },
  ...
  setLegendClickToggle: (spec, value) => {
    if (!value) {
      removeParam(spec, "__legend_sel__");
      return;
    }
    const colorField = findEncodingField(spec, "color");
    if (!colorField) return;
    setParam(spec, {
      name: "__legend_sel__",
      select: { type: "point", fields: [colorField.field] },
      bind: "legend",
    });
    // Bind opacity to selection (non-destructive: preserves existing opacity logic)
    walkEncoding(spec, "opacity", enc => {
...
function setDomainBound(spec, channel, idx, rawValue) {
  ...
  walkEncoding(spec, channel, enc => {
    ...
    if (isNaN(num) || rawValue === "") enc.scale.domain[idx] = null;
    else enc.scale.domain[idx] = num;
    if (enc.scale.domain[0] === null && enc.scale.domain[1] === null) delete enc.scale.domain;
  });
}
...
function initializeKnobs() {
  ...
  // Essentials group (knobs marked essential)
  ...
  // Group other knobs by group field
  const groups = {};
  ...
  for (const gname of sortedGroups) {
    ...
    const openGroups = ["Title", "X-Axis", "Y-Axis", "Legend"];
    details.open = openGroups.includes(gname);
    const summary = document.createElement("summary");
...
/* ============================================================
   ...
   ============================================================ */
function applyTheme(themeName, record) {
  const theme = THEMES[themeName];
  if (!theme) { setStatus("theme '" + themeName + "' not found"); return; }
  currentTheme = themeName;
  for (const k of KNOBS) {
    let v = theme.values[k.name];
...
function applyDimensionPreset(presetName, record) {
  ...
  if (presetName !== "custom") {
    ...
    setPath(currentSpec, "width", preset.width);
    setPath(currentSpec, "height", preset.height);
...
function switchTab(tabName) {
  ...
  if (tabName === "data") { renderDataTable(); renderSummaryStats(); }
  ...
  for (const key of ["layer", "hconcat", "vconcat", "concat"]) {
    if (Array.isArray(spec[key])) {
      for (const child of spec[key]) {
        const d = extractInlineData(child);
        if (d) return d;
      }
    }
  }
  if (spec.spec) return extractInlineData(spec.spec);
  return null;
}

function refreshDataCache() {
  const data = extractInlineData(currentSpec);
  _dataRows = Array.isArray(data) ? data.slice() : [];
...
function computeColumnStats(col) {
  const vals = [];
  for (const row of _dataRows) {
    const v = row[col];
  ...
  return {
    ...
    median: median,
    missing: _dataRows.length - n,
  };
}

function computeCategoricalStats(col) {
  const counts = {};
  let total = 0;
  let missing = 0;
  for (const row of _dataRows) {
  ...
  return {
    total: total, unique: unique, missing: missing,
...
function renderSummaryStats() {
  ...
  const perSeriesContainer = document.getElementById("perSeriesStatsContainer");
  if (!container) return;
  if (_dataRows.length === 0) {
    container.innerHTML = "";
    perSeriesContainer.innerHTML = "";
    return;
  }
  ...
  for (const col of _dataColumns) {
    ...
    if (numeric > sample.length * 0.5) numericCols.push(col);
    else categoricalCols.push(col);
  }

  let html = "<table class='stats-table'>";
  if (numericCols.length > 0) {
    html += "<tr><th>column</th><th>count</th><th>mean</th><th>std</th><th>min</th><th>median</th><th>max</th><th>missing</th></tr>";
  ...
  if (categoricalCols.length > 0) {
    html += "<tr><th colspan='8' style='text-align:left;background:#e8e8e8;'>Categorical columns</th></tr>";
    html += "<tr><th>column</th><th>total</th><th>unique</th><th>missing</th><th colspan='4'>top values</th></tr>";
    for (const col of categoricalCols) {
      const s = computeCategoricalStats(col);
      if (!s) continue;
      const top = s.top.map(([k, v]) => k + "(" + v + ")").join(", ");
      html += "<tr><td>" + col + "</td>"
            + "<td>" + s.total + "</td>"
            + "<td>" + s.unique + "</td>"
            + "<td>" + s.missing + "</td>"
            + "<td colspan='4' style='text-align:left;'>" + top + "</td></tr>";
    }
  }
  html += "</table>";
  container.innerHTML = html;

  // Per-series stats if color field detected
  const colorField = findEncodingField(currentSpec, "color");
  if (colorField && _dataRows.some(r => r[colorField.field] !== undefined)) {
    renderPerSeriesStats(colorField.field, numericCols);
  } else {
    perSeriesContainer.innerHTML = "";
  }
}

function renderPerSeriesStats(seriesField, numericCols) {
  const container = document.getElementById("perSeriesStatsContainer");
  if (!container) return;
  const groups = {};
  for (const row of _dataRows) {
  ...
  if (Object.keys(groups).length === 0) { container.innerHTML = ""; return; }

  let html = "<h3>Per-series stats (grouped by " + seriesField + ")</h3>";
  html += "<table class='stats-table'>";
  html += "<tr><th>" + seriesField + "</th><th>count</th>";
  for (const col of numericCols) html += "<th>" + col + " mean</th><th>" + col + " min</th><th>" + col + " max</th>";
  html += "</tr>";
  for (const [key, rows] of Object.entries(groups)) {
    html += "<tr><td>" + key + "</td><td>" + rows.length + "</td>";
    for (const col of numericCols) {
      const vals = rows.map(r => r[col]).filter(v => typeof v === "number" && !isNaN(v));
      if (vals.length === 0) { html += "<td>-</td><td>-</td><td>-</td>"; continue; }
...
function generateDataCode(spec) {
  const rows = extractInlineData(spec);
...
function renderMetadata() {
  ...
  const sections = [
    ...
    { title: "Chart configuration", rows: [
      ...
      ["Theme applied", currentTheme],
      ["Palette", currentPalette],
      ["Dimension preset", currentDimPreset],
...
function resetToTheme() {
  ...
  setStatus("reset to theme '" + currentTheme + "'");
}
...
function init() {
  ...
  populateSpecSheetSelect();
  applyTheme(currentTheme, false);
  if (INITIAL_PALETTE) applyPalette(INITIAL_PALETTE, false);
  if (INITIAL_DIM_PRESET) applyDimensionPreset(INITIAL_DIM_PRESET, false);
  ...
  updateSizeSummary();
  syncSelectors();
  // Initial population of Data / Code / Metadata tabs
...
def wrap_interactive_prism(
    ...
    chart_studio's generic wrap_interactive().
    ...
    PrismInteractiveResult
    """
    # map chart_type -> registered knobs
    chart_type_for_knobs = _prism_chart_type_to_mark(chart_type)

    # map PRISM dimensions to chart_studio preset
...
def _prism_chart_type_to_mark(prism_chart_type: str) -> str:
    """Map PRISM's chart_type names to chart_studio's mark keys."""
    mapping = {
        "multi_line":      "line",
        "line":            "line",
        "scatter":         "point",
...
DEFAULT_DEMO_OUTPUT_DIR = Path(__file__).parent / "chart_studio_demos"
...
def generate_demo_all(output_dir: Path, theme: str, palette: Optional[str]) -> List[Path]:
    ...
    for name in list_sample_names():
        ...
        result = wrap_interactive(
            ...
            title=f"chart_studio demo - {name} ({theme})",
        )
...
def generate_demo_one(name: str, output_dir: Path, theme: str, palette: Optional[str]) -> Path:
    ...
    result = wrap_interactive(
        spec=get_sample(name), theme=theme, palette=palette,
        output_path=out_path, title=f"chart_studio demo - {name} ({theme})",
    )
    print(f"  wrote {out_path.name}  chart_type={result.chart_type}")
...
# =============================================================================
# BUILT-IN SMOKE TESTS (python chart_studio.py test)
# =============================================================================
...
def run_smoke_tests() -> int:
    ...
    For the full unittest suite, see test_chart_studio.py.
    """
    failures: List[str] = []
    passes = 0
    ...
    print("chart_studio smoke tests")
    print("=" * 40)
...
def interactive_menu() -> int:
    print("")
    print(f"chart_studio interactive CLI (v{__version__})")
    print("=" * 40)
    while True:
        print("")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="chart_studio",
