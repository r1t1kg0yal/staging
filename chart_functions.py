...
Reconstructed from ``chart_functions.md`` (an OCR'd transcript of the
original PRISM module). Anything PRISM-specific (S3, error mailer,
presigned URLs, Gemini vision QC) is routed through
``chart_functions_utils.py`` so this file is plug-and-play across the
local repo and PRISM proper.
...
# ---------------------------------------------------------------------------
# PRISM-coupled utilities (single swap point)
# ---------------------------------------------------------------------------
# In local mode these resolve to filesystem-backed / no-op shims.
# In PRISM, ``chart_functions_utils.py`` is replaced with a module that
# re-exports the real implementations from ``ai_development.mcp.utils.*``.
from chart_functions_utils import (
    S3BucketManager,
    check_chart_quality,
    default_s3_manager,
    describe_image as _describe_image,
    generate_presigned_download_url,
    guess_units_from_name,
    send_error_email,
)

# Studio integration (sibling module, same in local and PRISM).
from chart_functions_studio import PrismInteractiveResult, wrap_interactive_prism
...
# ===========================================================================
# MODULE: VISION ANNOTATION PIPELINE
# ===========================================================================
...
def _build_annotation_prompt(
    ...
    """Build the Gemini vision prompt that requests annotation JSON.

    The prompt enumerates strict rules (text contrast, label placement,
    arrow sizing) and returns only a JSON object with an ``annotations``
    array. Pixel coordinates are top-left-origin (``0..width_px``,
    ``0..height_px``).
    """
    color_field = mapping.get("color", mapping.get("y", "value"))
    series_names = str(color_field)
    context_block = (
        f"EDITORIAL CONTEXT (what this chart is trying to communicate):\n{context}"
        if context
        else ""
    )

    return f"""You are a chart annotation engine. Examine this chart image and return ONLY a JSON object containing annotation instructions. No markdown, no prose -- just the JSON.

CHART METADATA (you do not need to infer these from the image):
  - Title: "{title or 'Untitled'}"
  - Subtitle: "{subtitle or 'None'}"
  - Chart type: {chart_type}
  - Series/columns: {series_names}
  - Image dimensions: {width_px} x {height_px} pixels (top-left origin)

ANNOTATION GUIDELINES FROM THE AUTHOR:
{guidelines}

{context_block}

RULES:
1. Return a JSON object with key "annotations" containing an array of annotation objects.
2. Use pixel coordinates with top-left (0,0) origin. All x in 0-{width_px}, y in 0-{height_px}.
3. Be conservative: 1-5 annotations for simple charts, 2-8 for dense charts.
4. Place labels in empty whitespace. Avoid covering legends, axis labels, or dense data.
5. Do NOT fabricate exact data values. Annotate direction, trends, inflection points.
6. Keep text short: 2-12 words, sentence case, thesis-oriented.
7. Include a confidence score (0.0-1.0) for each annotation.
8. Minimum 10px from image borders. Minimum 6px from key linework.
9. text_color must be dark (#000000-#444444). NEVER light grey or any colour with luminance > 40%.
10. Labels MUST sit in clear whitespace, not on top of data lines / bars / points.
11. Arrows: max length 30% of the smaller image dimension; stroke_width 2-3px (max 4).
    Arrows point TO data features, never across them.

PER-ANNOTATION SCHEMA:
{{
  "annotation_id": "string",
  "type": "arrow" | "text_label" | "line" | "circle" | "rectangle",
  "style": {{"stroke_color": "#RRGGBB", "stroke_width": 3, "font_size": 22,
             "text_color": "#111111", "label_background_opacity": 0.75,
             "arrowhead": "end"}},
  "geometry": {{...}},
  "text": "Short annotation text",
  "confidence": 0.85,
  "annotation_rationale": "Why this annotation"
}}

GEOMETRY BY TYPE:
  arrow:      {{"start": {{"x": N, "y": N}}, "end": {{"x": N, "y": N}}, "label_anchor": {{"x": N, "y": N}}}}
  text_label: {{"anchor": {{"x": N, "y": N}}, "box": {{"x": N, "y": N, "width": N, "height": N}}}}
  line:       {{"start": {{"x": N, "y": N}}, "end": {{"x": N, "y": N}}}}
  circle:     {{"center": {{"x": N, "y": N}}, "radius": N}}
  rectangle:  {{"x": N, "y": N, "width": N, "height": N}}

Return ONLY the JSON object.
""".strip()


def _parse_annotation_json(
    raw_response: str,
    width_px: int,
    height_px: int,
    confidence_threshold: float = 0.50,
) -> Optional[List[Dict[str, Any]]]:
    """Parse Gemini's annotation JSON response, tolerating common LLM quirks.

    Strategies tried in order:
      1. Extract from a fenced ```json ... ``` block.
      2. Find the outermost ``{...}``.
      3. Strip trailing commas before ``}``/``]``.

    Filters out annotations missing required keys, with unknown types, or
    below the confidence threshold. Clamps geometry coordinates to the
    image bounds.

    Returns:
        A list of validated annotation dicts, or ``None`` when parsing
        fails or no valid annotations remain.
    """
    ...
    if not text.startswith("{"):
        ...
        if brace_start == -1:
            logger.warning("[Annotation] No JSON object found in Gemini response")
            return None
    ...
    try:
        ...
        logger.warning("[Annotation] JSON parse error: %s", exc)
        return None

    annotations = data.get("annotations", [])
    if not isinstance(annotations, list):
        logger.warning("[Annotation] 'annotations' key is not a list")
        return None

    valid: List[Dict[str, Any]] = []
    required_fields = {"annotation_id", "type", "geometry"}
    allowed_types = {"arrow", "text_label", "line", "circle", "rectangle"}

    for ann in annotations:
        if not isinstance(ann, dict):
            continue
        if not required_fields.issubset(ann.keys()):
            continue
        if ann.get("type") not in allowed_types:
            continue
        conf = ann.get("confidence", 0.5)
        if conf < confidence_threshold:
            continue
        _clamp_geometry(ann.get("geometry", {}), width_px, height_px)
        valid.append(ann)

    return valid if valid else None


def _clamp_geometry(geom: Any, max_x: int, max_y: int) -> None:
    """Recursively clamp ``x`` / ``y`` / ``radius`` values in a geometry
    dict to the image bounds (mutates in place).
    """
    if not isinstance(geom, dict):
        return
    for key, val in geom.items():
        if isinstance(val, dict):
            _clamp_geometry(val, max_x, max_y)
        elif key == "x" and isinstance(val, (int, float)):
            geom[key] = max(0, min(val, max_x))
        elif key == "y" and isinstance(val, (int, float)):
            geom[key] = max(0, min(val, max_y))
        elif key == "radius" and isinstance(val, (int, float)):
            geom[key] = max(0, min(val, max_x, max_y))


def _hex_to_rgba(hex_color: str, opacity: float = 1.0) -> Tuple[int, int, int, int]:
    """Convert ``#RRGGBB`` to an ``(r, g, b, a)`` tuple with PIL-friendly ints."""
...
def _render_annotations(
    ...
    """Render annotation objects onto a PIL image, returning a new RGBA image.

    Supports ``arrow``, ``text_label``, ``line``, ``circle``, ``rectangle``.
    Uses Liberation Sans (vl-convert's base font) at the requested size
    so annotation text matches chart titles / axis labels visually.
    Semi-transparent fills go onto an alpha-composite overlay; opaque
    elements render directly onto the main image.
    """
    ...
    def _get_font(size: int, weight: str = "medium") -> Any:
        """Load Liberation Sans (or a metric-equivalent fallback)."""
        if weight == "bold":
            font_paths = [
    ...
    def _draw_arrowhead(
        ...
        draw_ctx.polygon([end, left, right], fill=color)
    ...
    for ann in annotations:
        ...
        stroke_color = style.get("stroke_color", "#111111")
        stroke_width = style.get("stroke_width", 3)
        text_color = style.get("text_color", "#111111")
        font_size = style.get("font_size", 22)
        font_weight = style.get("font_weight", "medium")
        bg_color = style.get("label_background_color", "#FFFFFF")
        bg_opacity = style.get("label_background_opacity", 0.75)
        fill_color = style.get("fill_color", "#888888")
        fill_opacity = style.get("fill_opacity", 0.8)
        font = _get_font(font_size, font_weight)

        try:
            if ann_type == "arrow":
                start = (geom["start"]["x"], geom["start"]["y"])
                end = (geom["end"]["x"], geom["end"]["y"])
                draw.line([start, end], fill=stroke_color, width=stroke_width)
                if style.get("arrowhead", "end") == "end":
                    _draw_arrowhead(
                        draw, start, end,
                        size=stroke_width * 4, color=stroke_color,
                    )
                anchor_geom = geom.get("label_anchor")
                if text and anchor_geom:
                    anchor = (anchor_geom["x"], anchor_geom["y"])
                    draw.text(anchor, text, fill=text_color, font=font)

            elif ann_type == "text_label":
                anchor_geom = geom.get("anchor", {})
                anchor = (anchor_geom.get("x", 0), anchor_geom.get("y", 0))
                box = geom.get("box")
                if box:
                    bg_rect = [
                        box["x"], box["y"],
                    ...
                    overlay_draw.rectangle(
                        bg_rect, fill=_hex_to_rgba(bg_color, bg_opacity)
                    )
                draw.text(anchor, text, fill=text_color, font=font)
                ...
                if fill_opacity > 0:
                    overlay_draw.ellipse(
                        bbox,
                        fill=_hex_to_rgba(fill_color, fill_opacity),
                ...
                if fill_opacity > 0:
                    ...
                    draw.rectangle(rect, outline=stroke_color, width=stroke_width)

        except (KeyError, TypeError) as exc:
            logger.warning(
                "[Annotation] Skipping annotation %s: %s",
...
def _apply_vision_annotations(
    ...
    """Run the optional Gemini vision-annotation pass on a chart PNG.

    Steps:
      1. Build a structured prompt and ship the PNG bytes to
         ``describe_image`` (PRISM: Gemini Flash; local: returns empty
         JSON).
      2. Parse the response into annotation objects with confidence
         filtering and bounds clamping.
      3. Render annotations onto a copy of the PNG.
      4. Save both the original (``..._original.png``) and the annotated
         (``png_path``) to S3.

    Returns:
        S3 path of the un-annotated original PNG (so make_chart() can
        reference it in ``ChartResult.original_png_path``), or ``None``
        if vision annotation was skipped or failed (caller should keep
        the original PNG only).
    """
    try:
        from PIL import Image  # noqa: F401 - lazy import

        prompt = _build_annotation_prompt(
            title, subtitle, guidelines, context,
            chart_type, mapping, width_px, height_px,
        )
        raw_response = _describe_image(png_bytes, prompt=prompt)
        annotations = _parse_annotation_json(
            raw_response, width_px=width_px, height_px=height_px
        )
        if not annotations:
            return None
        ...
        annotated = _render_annotations(img, annotations)

        buf = io.BytesIO()
        annotated.save(buf, format="PNG")
        annotated_bytes = buf.getvalue()

        # Save original under ..._original.png for backup.
        original_path = png_path.replace(".png", "_original.png")
        s3_manager.put(png_bytes, original_path)
        s3_manager.put(annotated_bytes, png_path)
        return original_path

    except Exception as exc:  # noqa: BLE001 - non-fatal fallback
        send_error_email(
            error_message=f"_apply_vision_annotations failed: {exc}",
            traceback_info=traceback.format_exc(),
            tool_name="chart_functions._apply_vision_annotations",
            tool_parameters={"png_path": png_path, "chart_type": chart_type},
            metadata={"error_type": type(exc).__name__},
            context="Vision annotation failed; original PNG retained.",
        )
        return None
...
class ChartResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "original_png_path": self.original_png_path,
    ...
    *,
    ...
    Args:
        ...
        x_label / y_label: Convenience aliases for ``mapping['x_title']`` /
            ``mapping['y_title']``.
        annotation_guidelines: Free-text instruction for the optional
            Gemini vision-annotation pass. Triggers the pass when set.
        annotation_context: Editorial context for vision annotations.
    ...
    s3_manager = s3_manager or default_s3_manager()
    ...
    # ---- Vision annotation (opt-in, non-fatal) -------------------------
    annotated = False
    original_png_path: Optional[str] = None
    if (
        annotation_guidelines
        and not png_save_failed
        and png_path
        and png_bytes
    ):
        try:
            original_png_path = _apply_vision_annotations(
                png_path=png_path,
                png_bytes=png_bytes,
                title=title,
                subtitle=subtitle,
            ...
            annotated = bool(original_png_path)
        except Exception as exc:  # noqa: BLE001
            warnings.append(f"Annotation failed (non-fatal): {exc}")

    # ---- Presigned download URL ----------------------------------------
    if png_path and not png_save_failed:
        try:
            download_url = generate_presigned_download_url(
                png_path, s3_manager=s3_manager
            )
        except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            try:
                editor_download_url = generate_presigned_download_url(
                    editor_html_s3_path, s3_manager=s3_manager
                )
            except Exception:  # noqa: BLE001
                pass
        except Exception as exc:  # noqa: BLE001
