...
- **Each desk voice is distinct** in Risk Positions. Vol (Ada Situ) talks gamma/duration/vega. UST (Brandon Brown/April Li) talks cash cheapness, curve sectors, RV. Short Macro (Brian Bingham) talks FOMC pricing, policy path, SOFR structures. Swaps (Ross Harmon/Victoria Otero) talks swap spreads, invoice, issuance backdrop.
...
### 8. Risk Positions
Always 4 entries, numbered, each with a desk head name and brief view:
...
Each WTW is different because the market is different each week. PRISM has a general state of the world, but the human must specify the exact content for this week's piece. The human should fill in the following:

### HUMAN INPUT SECTION

```
DATE: [publication date, e.g., "10 April 2026"]
AUTHORS: [list of 2-5 names]

--- FCI ---
FCI_LEVEL: [e.g., 98.68]
FCI_WOW: [e.g., "23bps EASIER"]
FCI_YTD: [e.g., "19bps TIGHTER"]
FCI_FROM_JAN_PRIOR: [e.g., "65bps EASIER FROM 01Jan25"]

--- THE WEEK THAT WAS ---
DOMINANT_THEME: [1-2 sentences on what dominated the week, e.g., "US-Iran ceasefire saga and its ripple effects across oil, rates, and risk sentiment"]
DAY_BY_DAY: [Key events per day -- what happened Monday through Friday, including:
  - Geopolitical headlines
  - Data releases (with exact prints vs consensus vs GS expectations)
  - Auction results (sizes, tails/thrus, bid metrics)
  - Specific bp moves in key tenors and curve expressions
  - Equity/oil/FX price action where it drove rates
  - Fed/ECB/BOE/BOJ commentary
  - Volume context (% of average, thin/heavy)
]
GIR_FORECAST_CHANGES: [any GS Economics forecast changes this week, or "none"]

--- THE WEEK THAT WILL BE ---
KEY_EVENTS: [Ordered by importance:
  - Marquee data release(s) with consensus/prior
  - Fed speakers (names + days)
  - Geopolitical catalysts to watch
  - Technical considerations (month-end, quarter-end, blackout)
  - Global events (ECB, BOE, BOJ, EZ CPI, Tankan, etc.)
]

--- SUPPLY ---
USD_IG: [estimated issuance, e.g., "25bn"]
UST: [auction calendar with sizes and dates]
EUR: [auction calendar]
UK: [auction calendar]
JPY: [auction calendar]

--- FLOWS ---
OVERALL_TONE: [light/heavy/mixed, conviction level]
DURATION_FLOWS: [who was doing what in duration -- CTA, RM, HF]
CURVE_FLOWS: [steepener/flattener activity, which sectors]
SPREAD_FLOWS: [swap spread activity]
VOL_FLOWS: [demand dynamics in vol space]
INFLATION_FLOWS: [breakeven/TIPS activity, if notable]
OTHER_FLOW_NOTES: [anything else -- e.g., month-end extension, IG issuance absorption]

--- RISK POSITIONS ---
VOL_TRADER: [name + 1-3 sentence view]
UST_TRADER: [name + 1-3 sentence view]
SHORT_MACRO_TRADER: [name + 1-3 sentence view, including specific trade expressions if any]
SWAPS_TRADER: [name + 1-3 sentence view]

--- NICE TO KNOW ---
TOPIC: [specific topic for this week's deep dive, e.g., "FCI tightening case study", "5y inflation basis at extremes", "foreign official sector selling", "prime book positioning"]
TOPIC_DATA: [supporting data, levels, charts to reference]
TOPIC_NARRATIVE: [key analytical point or observation to make, 2-5 sentences of the core argument]

--- CROSS-ASSET SUMMARY (optional) ---
INCLUDE: [yes/no]
ASSET_MOVES: [if yes, provide WoW moves: energy %, rates bp, equities %, FX %, credit bp, VIX %]
```
