...
LLM / programmatic use:
  ...
  Domain getters: get_cusips, get_unique_cusips, get_full_cusip_universe, get_debt_to_penny, get_mts_table, get_dts_table, etc.
...
def _menu_cusips() -> None:
    print("\n--- CUSIPs ---")
    print("1. Full universe (upcoming_auctions + frn_daily_indexes)")
    print("2. List unique CUSIPs (upcoming only)")
    print("3. List unique CUSIPs (filter by security type)")
    print("4. Full CUSIP records (with auction dates)")
    print("0. Back")
    ...
    if choice == "1":
        ...
        print(f"Full universe: {len(cusips)} unique CUSIPs")
...
def _menu_buybacks() -> None:
    print("\n  Treasury Buyback Operations")
    print("  1. All buybacks (latest 30)")
    print("  2. Liquidity Support only")
    print("  3. Cash Management only")
    print("  4. TIPS buybacks only")
    print("  5. With results only")
    choice = input("  Choice: ").strip()
    op_type = None
    sec_type = None
    results_only = False
    if choice == "2":
        op_type = "Liquidity Support"
    elif choice == "3":
        op_type = "Cash Management"
    elif choice == "4":
        sec_type = "TIPS"
    elif choice == "5":
        results_only = True
    try:
        data = get_buybacks(operation_type=op_type, security_type=sec_type, with_results_only=results_only)
        print(f"\n  {len(data)} buyback operations")
        print(f"  {'Date':<12} {'Type':<22} {'Security':<18} {'Bucket':<15} {'Offered':>16} {'Accepted':>16}")
        print(f"  {'-'*12} {'-'*22} {'-'*18} {'-'*15} {'-'*16} {'-'*16}")
        for r in data[:30]:
            date = r.get("operation_date", "")[:10]
            otype = r.get("operation_type", "")[:22]
            sec = r.get("security_type", "")[:18]
            bucket = r.get("maturity_bucket", "")[:15]
            offered = r.get("total_par_amt_offered", "null")
            accepted = r.get("total_par_amt_accepted", "null")
            off_s = f"${float(offered)/1e9:.1f}B" if offered and offered != "null" else "--"
            acc_s = f"${float(accepted)/1e9:.1f}B" if accepted and accepted != "null" else "--"
            print(f"  {date:<12} {otype:<22} {sec:<18} {bucket:<15} {off_s:>16} {acc_s:>16}")
    except Exception as e:
        print(f"  Error: {e}")


def _menu_auctions_data() -> None:
    sec_type = input("  Security type (Bill/Note/Bond/TIPS/FRN, or blank for all): ").strip() or None
    try:
        data = get_auctions_data(security_type=sec_type, max_pages=1)
        _print_table(data, limit=20)
    except Exception as e:
        print(f"  Error: {e}")
...
def _interactive_main() -> None:
    while True:
        ...
        print("8. Treasury buybacks")
        print("9. Auction data (pricing, bid-to-cover)")
        print("0. Exit")
        choice = input("Choice: ").strip()
        if choice == "0":
            break
...
def main() -> None:
    ...
    p_cusips.add_argument("--full-universe", action="store_true", help="Full universe: upcoming_auctions + frn_daily_indexes (max API coverage)")
    ...
    p_cusips.add_argument("--progress", action="store_true", help="Show progress for full-universe fetch")
    ...
    p_bb = subparsers.add_parser("buybacks", help="Treasury buyback operations")
    p_bb.add_argument("--operation-type", choices=["Cash Management", "Liquidity Support"], help="Filter by operation type")
    p_bb.add_argument("--security-type", choices=["Nominal Coupons", "TIPS"], help="Filter by security type")
    p_bb.add_argument("--from-date", help="From date (YYYY-MM-DD)")
    p_bb.add_argument("--to-date", help="To date (YYYY-MM-DD)")
    p_bb.add_argument("--results-only", action="store_true", help="Only operations with results")
    p_bb.add_argument("--json", action="store_true", help="Output JSON")
    p_bb.add_argument("--limit", type=int, default=30, help="Max rows to display")
    p_bb.add_argument("--fields", help="Comma-separated field names")
    p_bb.add_argument("--filter", help="Raw filter expression")

    p_aq = subparsers.add_parser("auctions-data", help="Treasury securities auction data (pricing, rates, bid-to-cover)")
    p_aq.add_argument("--security-type", "-s", help="Filter by security type (Bill, Note, Bond, TIPS, FRN)")
    p_aq.add_argument("--cusip", help="Filter by CUSIP")
    p_aq.add_argument("--from-date", help="From date (YYYY-MM-DD)")
    p_aq.add_argument("--to-date", help="To date (YYYY-MM-DD)")
    p_aq.add_argument("--json", action="store_true", help="Output JSON")
    p_aq.add_argument("--limit", type=int, default=20, help="Max rows to display")
    p_aq.add_argument("--fields", help="Comma-separated field names")
    p_aq.add_argument("--filter", help="Raw filter expression")
    ...
    if args.command == "cusips":
        if getattr(args, "full_universe", False):
            cusips = get_full_cusip_universe(show_progress=getattr(args, "progress", False))
            data = [{"cusip": c} for c in cusips]
            if not args.json:
                print(f"Full CUSIP universe: {len(data)} unique CUSIPs (upcoming_auctions + frn_daily_indexes)")
        ...
        data = get_buybacks(
            ...
            with_results_only=getattr(args, "results_only", False),
        ...
        if args.json:
            ...
            print(f"\n  Treasury Buyback Operations ({len(data)} results)")
            print("  " + "=" * 100)
            print(f"  {'Date':<12} {'Type':<22} {'Security':<18} {'Bucket':<15} "
                  f"{'Offered':>16} {'Accepted':>16} {'Issues':>7}")
            print(f"  {'-'*12} {'-'*22} {'-'*18} {'-'*15} {'-'*16} {'-'*16} {'-'*7}")
            for r in data[:args.limit]:
                ...
                offered = r.get("total_par_amt_offered", "null")
                accepted = r.get("total_par_amt_accepted", "null")
                issues = r.get("nbr_issues_accepted", "null")
                off_s = f"${float(offered)/1e9:.1f}B" if offered and offered != "null" else "--"
                acc_s = f"${float(accepted)/1e9:.1f}B" if accepted and accepted != "null" else "--"
                iss_s = str(issues) if issues and issues != "null" else "--"
                print(f"  {date:<12} {otype:<22} {sec:<18} {bucket:<15} "
                      f"{off_s:>16} {acc_s:>16} {iss_s:>7}")
            if len(data) > args.limit:
                print(f"  ... {len(data) - args.limit} more")
            print()

    elif args.command == "auctions-data":
        flds = [x.strip() for x in args.fields.split(",")] if getattr(args, "fields", None) else None
        data = get_auctions_data(
            security_type=args.security_type,
            from_date=args.from_date,
            to_date=args.to_date,
            cusip=getattr(args, "cusip", None),
            filter_expr=getattr(args, "filter", None),
            fields=flds,
